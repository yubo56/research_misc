% rm -f LK90_plots/*.png; for i in 7_3vec_eta_cropped 8bin_comp_en2 8bin_comp 8finite_qslfs M1_M2_thetasl_thetae_70 5_dWs_inner 5_dWs 6bin_comp 6bin_comp_zoom 4qslscan/qslscan 4sims_scan/deviations_one 4sims/4sim_90_200_zoom 4sims/4sim_90_350_supp 4sims/4sim_90_350_zoom 4sims/4sim_90_350 7_3vec_cropped 4sims/good_quants; do cp ../../scripts/lk90/$i.png LK90_plots; done
    \documentclass[
        twocolumn,
        twocolappendix
    ]{aastex63}
...
\begin{figure}
    ...
    $90^\circ$. The black dashed line in the lower panel shows
...
\end{equation}
In the special case where the initial binary satisfies $\overline{\Omega}_{\rm
...
Numerically, we find that $\overline{\Omega}_{\rm e} \lesssim \Omega_{\rm
LK}$ for most regions of parameter space (see Fig.~\ref{fig:dWs}, and recall
that LK-induced mergers only complete within a Hubble time when $I_{\min}
\approx 90^\circ$). Accordingly, we restrict our analysis to resonances with the
$N = 1$ component. For simplicity, we also ignore the modulation of the forcing
...
Fig.~\ref{fig:4sim_90_350_supp}. We see that the total effect of the harmonic
terms never exceeds a few degrees.
...
final deviations $\abs{\Delta \bar{\theta}_{\rm e}}_{\rm f}$ very well (see
Fig.~\ref{fig:deviations} for $I_{\rm i} = 90.35^\circ$). This suggests that
...
In LL17, a different parameter regime is considered, where the inner binary is
sufficiently close in ($\sim 0.1\;\mathrm{AU}$) that it can merge in isolation
via GW radiation, given by: $m_1 = m_2 = m_3 = 30M_{\odot}$, $a_{\rm in} =
0.1\;\mathrm{AU}$, $\tilde{a}_{\rm out} = 3\;\mathrm{AU}$, and $e_{\rm out} =
0$. Note that $m_3$ here is not an SMBH\@. However, our results can still be
applied judiciously to this parameter regime and yield interesting insights.

First, we recall the $\theta_{\rm sl, f}$ distribution obtained via
...
remains circular. Very near $I_{\rm i} \approx 90^\circ$, the data are offset somewhat
...
$\theta_{\rm sl, f}$, as seen in Fig.~\ref{fig:bin_comp_zoom}.

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{LK90_plots/6bin_comp.png}
    \caption{Plot of $\theta_{\rm sl, f}$ for the LK-enhanced parameter
    regime, i.e.\ $m_1 = m_2 = m_3 = 30M_{\odot}$, $a_{\rm in} =
    0.1\;\mathrm{AU}$, $\tilde{a}_{\rm out} = 3\;\mathrm{AU}$, and $e_{\rm out}
    = 0$. Conservation of $\bar{\theta}_{\rm e}$ gives the green line. Agreement
    near $I_{\rm i} = 90^\circ$ is good when accounting for the effects of a finite
    $L_{\rm out}$ (see Fig.~\ref{fig:bin_comp_zoom}). For $I_{\rm i}$ in the two
    intervals $[40^\circ, 80^\circ]$ and $[100^\circ, 140^\circ]$, a further
    effect causes $\theta_{\rm sl, f}$ to fluctuate
...
\begin{figure}
    ...
    \caption{Zoomed in version of Fig.~\ref{fig:bin_comp} near $I_{\rm i} \approx
    90^\circ$ while adding an $I_{\rm i}$ offset to account for differences between
    ...
    the transition to a larger $\theta_{\rm sl, f}$ spread roughly follows
...
Fig.~\ref{fig:dWs_inner}, so this explanation is consistent with the ranges of
inclinations that exhibit volatile $\theta_{\rm sl, f}$ behavior. We forgo
further investigation of this mechanism because it is not expected to play an
important role in any LK-induced BH binary mergers for reasons discussed below.
...
\begin{figure}
    ...
    nonzero, with the same conventions as those in Fig.~\ref{fig:3vec}. Here,
    $\bm{L}_{\rm out}$ is not fixed, but $\bm{L}_{\rm tot} \equiv \bm{L} +
    \bm{L}_{\rm out}$ is. Note that the coordinate system is now oriented with
    $\uv{z} \propto \bm{L}_{\rm tot}$.}\label{fig:3vec_eta}
\end{figure}
...
\begin{equation}
    \eta = \at{\frac{L}{L_{\rm out}}}_{e = e_{\rm out} = 0} =
        \frac{\mu}{\mu_{\rm out}}
...
inclination angle $\bar{I}_{\rm e-tot}$ (Fig.~\ref{fig:3vec_eta}), which can be
...
Then $\bar{\theta}_{\rm e, i} = \abs{I_{\rm tot, i} - \bar{I}_{\rm e-tot, i}}$
(see Fig.~\ref{fig:3vec_eta}), and so
\begin{equation}
    \theta_{\rm sl, f} = \abs{I_{\rm tot, i} - \bar{I}_{\rm e, i}}.
        \label{eq:qslf_eta_pred}
\end{equation} Suppose additionally that $\abs{\overline{\Omega}_{\rm L, i}} \gg
\overline{\Omega}_{\rm SL, i}$, then $\overline{\Omega}_{\rm e, i}$ is either
parallel or anti-parallel to $\bm{L}_{\rm tot}$ depending on whether
$\overline{\Omega}_{\rm L}$ is positive or negative. The sign of
$\overline{\Omega}_{\rm L}$ is positive if $I_{\rm i} < I_{\lim}$ and negative
if $I_{\rm i} > I_{\lim}$, where $I_{\lim}$ is the starting mutual inclination
for which the maximum eccentricity is largest. Note that even though
$\Omega_{\rm L}$ changes signs at $I = 90^\circ$ [see Eq.~\eqref{eq:dWdt_eta}],
$\overline{\Omega}_{\rm L}$ changes sign at $I = I_{\lim}$. This is because for
$I < I_{\lim}$, the eccentricity maximum occurs when $I < 90^\circ$, and
$\overline{\Omega}_{\rm L}$ is dominated by the contribution around the
eccentricity maximum. $I_{\rm \lim, i}$ is given by \citep{bin2}
\begin{equation}
    \cos I_{\lim} = \frac{\eta}{2}\p{\frac{4}{5}j_{\lim}^2 - 1},
\end{equation}
where $j_{\lim}$ satisfies
\begin{equation}
    \frac{3}{8}\p{j_{\lim}^2 - 1}\s{-3 + \frac{\eta^2}{4}
        \p{\frac{4}{5}j_{\lim}^2 - 1}} + \epsilon_{\rm GR}\p{1 - j_{\lim}^{-1}}
        = 0.
\end{equation}
...
\begin{equation}
    \theta_{\rm sl, f} =
    \begin{cases}
        I_{\rm tot, i} & I_{\rm i} < I_{\lim},\\
        180^\circ - I_{\rm tot, i} & I_{\rm i} > I_{\lim}.
    \end{cases}
\end{equation}

Fig.~\ref{fig:finite_qslfscan} shows that Eq.~\eqref{eq:qslf_eta_pred}
accurately predicts $\theta_{\rm sl, f}$ in the finite-$\eta$, LK-induced regime
when conservation of $\bar{\theta}_{\rm e}$ is good. Furthermore, deviations
from exact $\bar{\theta}_{\rm e}$ conservation are well described by
Eq.~\eqref{eq:qslf_plot_black}, the prediction of the theory in
Section~\ref{s:fast_merger}. Unlike the $\eta = 0$ case
(Fig.~\ref{fig:qslscan}), $\theta_{\rm sl, f}$ is not symmetric about
$I_{\lim}$. This is because $\abs{\Omega}_{\rm L}$ is not exactly equal on
either side of $I_{\lim}$. Additionally, unlike in the $\eta = 0$ case, the
...
\begin{figure}
    ...
    \caption{Same as Fig.~\ref{fig:qslscan} but for $m_3 = 30M_{\odot}$ and
    $\tilde{a}_3 = 4500\;\mathrm{AU}$. The vertical black line denotes $I_{\rm
    i} = I_{\lim}$, the inclination for which the maximum eccentricity is
    largest, and the two horizontal lines denote the predictions of
...
\begin{figure}
    ...
    \caption{Various dynamical quantities as a function of $I_{\rm i}$ for the
    LK-enhanced parameter regime, i.e.\ $m_1 = m_2 = m_3 = 30M_{\odot}$, $a =
    0.1\;\mathrm{AU}$, $e_{\rm i} = 10^{-3}$, $\tilde{a}_{\rm out} =
    3\;\mathrm{AU}$, and $e_{\rm out} = 0$. In the first panel, we show the
    magnitude of the adiabaticity parameter $\abs{\mathcal{A}}$. In the second
    panel, we show the ratio of the magnitude of the averaged precession vector
    to the LK frequency; we expect resonant effects (see Section~TODO) when
    $\overline{\Omega}_{\rm e} \approx N\Omega_{\rm LK}$. The third panel shows
    the magnitude of the angle between the eigenvector of the monodromy matrix
    (see Section~\ref{ss:monodromy}) and the averaged precession vector, as well
    as the effect of the first two resonant harmonics
    [Eq.~\eqref{eq:harmonic_dqeff}]. Finally, in the final panel we show
    $\theta_{\rm sl, f}$ as a function of $I_{\rm i}$ from simulations (blue
    dots; LL17) and from conservation of $\bar{\theta}_{\rm e}$ (red line). The
    shaded red region is the predicted oscillation in $\bar{\theta}_{\rm e}$ due
    to the numerically calculated angle between $\bm{R}$ and
    $\overline{\bm{\Omega}}_{\rm e}$.}\label{fig:finite_bin_comp}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{LK90_plots/8bin_comp_en2.png}
    \caption{Same as Fig.~\ref{fig:finite_bin_comp} but for $e_{\rm i} =
    10^{-2}$. The location of the resonances moves away from $I_{\rm i} =
    90^\circ$ as the minimum eccentricity
    increases.}\label{fig:finite_bin_comp2}
\end{figure}
...
\begin{equation}
    \theta_{\rm sl, f} =
    \begin{cases}
        \theta_{\rm s-tot, i} & I < I_{\lim},\\
        180^\circ - \theta_{\rm s-tot, i} & I > I_{\lim}.
    \end{cases}
\end{equation}
...
\section{Alternative \S5 (rough)}

In the previous section, we assumed that the evolution of $\bm{S}$ is well
described as precession about $\overline{\bm{\Omega}}_{\rm e}$, the averaged
effective precession vector. In this section, we prove that this prescription is
valid for most relevant regions of parameter space.

For simplicity, we neglect the effects of GW radiation. In this case, the spin
dynamics are exactly periodic, and we showed in Section~\ref{ss:monodromy} that
the spin vector evolution can be described by an iterative map, the monodromy
matrix $\tilde{\bm{M}}$. We seek to express $\tilde{\bm{M}}$ in closed form.
Consider the EOM
\begin{equation}
    \rd{\bm{S}}{t} = \bm{\Omega}_{\rm e}\times \bm{S}.
\end{equation}
Let's consider the matrix operator $\tilde{\bm{A}}\bm{S} = \bm{\Omega}_{\rm e}
\times \bm{S}$, i.e.\ $\tilde{A}_{\rm ik} = \epsilon_{ijk}\Omega_{\rm e, j}$.
Then we have a simple first-order ODE
\begin{equation}
    \rd{\bm{S}}{t} = \tilde{\bm{A}}\bm{S}.
\end{equation}
When $\tilde{\bm{A}}$ is a function of $t$ as well, the usual solution via an
integrating factor is not general. We can try this and we will find
\begin{align}
    \rd{}{t}\s{e^{\Phi}\bm{S}}
        = e^{\Phi} \rd{\bm{S}}{t}
            -\tilde{\bm{A}}e^{\Phi}\bm{S} \neq 0,
\end{align}
where $\Phi \equiv \int -\tilde{\bm{A}}\;\mathrm{d}t$. The failure arises
because $\tilde{\bm{A}}$ and $e^{\Phi}$ do not commute in general. For instance,
when $\overline{\bm{\Omega}}_{\rm e}$ nutates significantly, $e^\Phi$ and
$\tilde{\bm{A}}$ do not commute because rotations do not commute.

If $\tilde{\bm{A}}$ and $e^{\Phi}$ commute, e.g.\ when
$\overline{\bm{\Omega}}_{\rm e}$ changes magnitude but not direction, then the
integrating factor above solves the system, and we obtain that
\begin{align}
    \bm{S}\p{t + T} &=
            \exp\s{\int\limits_0^T \tilde{\bm{A}}\;\mathrm{d}t}\bm{S}(t),
            \nonumber\\
        &= \exp\s{\ev{\tilde{\bm{A}}}T}\bm{S}(t).
\end{align}
This exponential operator is just the form for a rotation about
$\uv{\overline{\bm{\Omega}}}_{\rm e}$ with magnitude $\overline{\Omega}_{\rm
e}T$, and so we have a closed form solution for the monodromy matrix.

We can then ask what the leading order correction to this is, when
$\s{\tilde{\bm{A}}, e^{\Phi}}$ is ``small'' in some sense. We accomplish this by
making a new ansatz for the integrating factor, requiring
\begin{align}
    0 &= \rd{}{t}\s{e^{\Phi'}\bm{S}},\\
    \Phi' &= \int \tilde{\bm{A}} + \tilde{\bm{B}}\;\mathrm{d}t.
\end{align}
This can be solved to obtain the requirement that
\begin{equation}
    \tilde{\bm{B}} = e^{-\Phi}\s{\tilde{\bm{A}}, e^{\Phi}}
        + \mathcal{O}\p{\tilde{\bm{B}}}.
\end{equation}
This implies that the monodromy matrix is given by
\begin{align}
    \tilde{\bm{M}} &= \exp\s{\int\limits_0^T
            \tilde{\bm{A}} + \tilde{\bm{B}}\;\mathrm{d}t},\nonumber\\
        &= \exp\p{\ev{\tilde{\bm{A}}}T}\exp\p{\ev{\tilde{\bm{B}}}T}.
\end{align}
It is now clear that the requirement that $\s{\tilde{\bm{A}}, e^\Phi}$ be small
amounts to $\exp\p{\ev{\tilde{\bm{B}}}T}$ having a much smaller effect on
$\bm{S}$ than $\exp\p{\ev{\tilde{\bm{A}}}T}$. Generally, this is expected to be
satisfied, since the range of nutation of $\overline{\bm{\Omega}}_{\rm e}$ is
limited, so $\tilde{\bm{B}}$ is generally small. However, note that when
$\overline{\Omega}_{\rm e} \approx N\Omega_{\rm LK}$,
$\exp\p{\ev{\tilde{\bm{A}}}T}$ becomes very near the identity (it induces a
rotation about $\uv{\overline{\bm{\Omega}}}_{\rm e}$ by angle $2\pi N$). In
summary, this proves that the prescription introduced in
Eq.~\eqref{eq:dsdt_0only} is valid except when both $\overline{\bm{\Omega}}_{\rm
e}$ is nutating significantly (requiring $\mathcal{A} \simeq 1$) and when
$\overline{\Omega}_{\rm e} \simeq N\Omega_{\rm LK}$.

We can now understand these requirements in terms of the physical parameters of
the system. $\overline{\Omega}_{\rm e} / \Omega_{\rm LK}$ can be numerically
calculated for the fiducial parameter regime, and its behavior is shown in
Fig.~\ref{fig:dWs}. It can be seen that very near $I = 90^\circ$,
$\overline{\Omega}_{\rm e} / \Omega_{\rm LK} \lesssim 0.5$ is satisfied for all
eccentricities, and for $e_{\min} \gtrsim 0.3$, $\overline{\Omega}_{\rm e} /
\Omega_{\rm LK} \lesssim 0.5$ as well. As such, for the fiducial simulation (see
Fig.~\ref{fig:4sim_90_350}), we always expect $\overline{\Omega}_{\rm e} /
\Omega_{\rm LK} \lesssim 0.5$, which Fig.~\ref{fig:4sim_90_350_supp} shows to be
the case. In fact, since $e_{\min}$ and $\mathcal{A}$ change over similar
timescales ($\sim t_{\rm GW}$), the only way that Eq.~\eqref{eq:dsdt_0only}
becomes invalid is when $\mathcal{A}_{\rm i} \simeq 1$. This is not the case for
LK-induced mergers, as GR effects are too weak in these systems to force the
inner binary to merge without LK oscillations.

\subsection{Quantitative Analysis}

Explicitly evaluating the commutator in the definition of $\tilde{\bm{M}}$ is
difficult to do in general. In this section, we show that the behavior of the
axis of rotation of $\tilde{\bm{M}}$ can be understood.

Remainder of the old Section 5, resonances

In conclusion, we find that the effect of these resonances is to tilt the
effective rotation axis away from $\overline{\bm{\Omega}}_{\rm e}$

\section{Alternative \S6}

For LK-enhanced mergers (TODO elaborate), the inner binary is significantly more
compact, and $\mathcal{A}_{\rm i} \simeq 1$ is satisfied for much of parameter
space (see Fig.~\ref{fig:finite_bin_comp}). For such systems, significant
deviation from the predictions of Section~\ref{s:fast_merger} are expected. In
the bottom panel of Fig.~\ref{fig:finite_bin_comp}, we show the results of
simulations in the blue dots, and the prediction $\theta_{\rm e, i} =
\theta_{\rm sl, f}$ in the solid red line: it is clear that for broad ranges of
$I_{\rm i}$ the prediction is very poor. In this section, we show that the
resonances described in the previous section are responsible for generating
these deviations from $\bar{\theta}_{\rm e}$ conservation.

In the first panel of Fig.~\ref{fig:finite_bin_comp}, we show the adiabaticity
parameter $\mathcal{A}$. It is clear that when $\mathcal{A} \gg 1$, the
conservation of $\bar{\theta}_{\rm e}$ is very accurate (near $I_{\rm i} =
90^\circ$). Conservation is also very good for $I_{\rm i} \lesssim 50^\circ$ and
$I_{\rm i} \gtrsim 130^\circ$ since there is no eccentricity excitation and thus
all $N \geq 1$ Fourier components of $\bm{\Omega}_{\rm e}$ are zero.

In the second panel of Fig.~\ref{fig:finite_bin_comp}, we show
$\overline{\Omega}_{\rm e} / \Omega_{\rm LK}$. According to the results of the
previous section, we expect large deviations from $\bar{\theta}_{\rm e, i}$
conservation when $\overline{\Omega}_{\rm e} = N\Omega_{\rm LK}$. This can be
observed to be the case upon comparison with the bottom panel.

In the third panel of Fig.~\ref{fig:finite_bin_comp}, we show the misalignment
angle between $\bm{R}$ and $\overline{\bm{\Omega}}_{\rm e}$ at initial
conditions. We see that $\bm{R}$ and $\overline{\bm{\Omega}}_{\rm e}$ are near
parallel exaclty where conservation of $\bar{\theta}_{\rm e, i}$ is accurate,
but there are significant deviations from alignment. The green and blue lines
are given by evaluating Eq.~\eqref{eq:harmonic_dqeff} for initial conditions for
the $N = 1$ and $N = 2$ harmonics respectively; we see that they capture the
misalignment angle between $\bm{R}$ and $\overline{\bm{\Omega}}_{\rm e}$ well.

Taken all together, these show that there can be large $\bar{\theta}_{\rm e}$
variations in the LK-enhanced regime caused by the resonances described in the
previous section. However, these resonances are large only for particular values
of $I_{\rm i}$, while the deviations of the data from the predicted curve in the
bottom panel of Fig.~\ref{fig:finite_bin_comp} are not localized to these values
of $I_{\rm i}$. In Fig.~\ref{fig:finite_bin_comp2}, we show the effect of using
$e_{\rm i} = 10^{-2}$ instead. It is clear that the resonances move farther away
from $90^\circ$.

Finally, we observe that in the continuum limit, conservation of phase space
area would imply $\theta_{\rm R, i} = \theta_{\rm sl, f}$ where $\theta_{\rm R}$
is the angle between $\bm{R}$ and $\bm{S}$. However, this is clearly not
observed in Fig.~\ref{fig:finite_bin_comp}. This is likely due to the narrowness
of the resonances which ensures adiabatic passage (that would conserve phase
space area) is impossible, though a detailed calculation is beyond the scope of
this paper.
