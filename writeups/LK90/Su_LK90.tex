...
\title{Spin Dynamics in Hierarchical Black Hole Triples: An Adiabatic Invariant}
\author{Yubo Su}
...
\textcolor{Corr}{The recently-announced LIGO detection GW190521 \citep{190521}
has $\chi_{\rm eff} = 0.08_{-0.38}^{+0.27}$ while $\chi_{\rm p} =
0.68_{-0.37}^{+0.25}$. The spin-orbit misalignment angles of the two merging BHs
are reported to be $81_{-53}^{+64}$ and $85_{-55}^{+57}$ degrees respectively.}
...
inclination of the tertiary orbit $I_0$ (relative to the inner orbit) is
varied. Note that for rapid mergers (when $I_0$ is close to $90^\circ$),
the attractor does not exist; as $I_0$ deviates more from $90^\circ$, the
...
\begin{figure}
    ...
    $a_0 = 100\;\mathrm{AU}$, $e_0 = 0.001$, $I_0 =
    ...
    sl}$ evolves to $\sim 90^\circ$ as $a$ decays to smaller values,
    \textcolor{Corr}{and we stop the simulation when $a = 0.5\;\mathrm{AU}$ as
    $\theta_{\rm sl}$ has converged to a constant
    value}.}\label{fig:4sim_90_350}
\end{figure}
\begin{figure}
    \centering
...
our fiducial parameters (described in the figure caption). \textcolor{Corr}{The
...
write $\bv{S} \equiv \uv{S}$ for brevity (i.e.\ $\bv{S}$ is a unit vector).
...
LK oscillation freezes, we have $\bv{\Omega}_{\rm e} \simeq
\overline{\bv{\Omega}}_{\rm e}$ (and $\bv{\Omega}_{\rm eN} \simeq 0$ for $N \geq
...
angle between $\overline{\bv{\Omega}}_{\rm e}$ and $\bv{L}_{\rm out}$ as shown in
...
out}$ and $\bv{L}$ (see Fig.~\ref{fig:3vec}). In this reference frame, the spin
...
\end{align}
where $\Omega_{\rm eN}$ is the magnitude of the vector $\bv{\Omega}_{\rm eN}$
[see Eq.~\eqref{eq:dsdt_fullft}] and $\Delta I_{\rm eN} = I_{\rm eN} -
\bar{I}_{\rm e}$ where $I_{\rm eN}$ is the angle between $\bv{\Omega}_{\rm eN}$
and $\bv{L}_{\rm out}$ (see Fig.~\ref{fig:3vec}).
...
\begin{figure}
    ...
    the zeroth and first Fourier components of $\bv{\Omega}_{\rm e}$ to the LK
    ...
    Eq.~\eqref{eq:nonad_dqeff}] and due to the resonance $\overline{\Omega}_{\rm
    e} \approx \Omega_{\rm LK}$ [blue,
...
$\overline{\bv{\Omega}}_{\rm e}$ is parallel to $\bv{L}$, and so
...
$\bv{L}$ is constant. Thus, $\bv{L}$ is fixed as $\bv{S}$ precesses around it,
...
can be understood when neglecting the $N \geq 1$ Fourier harmonics in
Eq.~\eqref{eq:dsdt_fullft}. In this section, we study the effects of these
neglected terms and show that they have a negligible effect in the LK-induced
regime. Separately, we will also consider the LK-enhanced regime and show that
these Fourier harmonics play a dominant role in shaping the $\theta_{\rm sl, f}$
distribution.
...
We first restrict our attention to the effect of just the $N$ Fourier harmonic,
and Eq.~\eqref{eq:formal_sol_gen} further simplifies to
\begin{align}
...
frequency and a driving term. We can begin to understand the dynamics of
$S_{\perp}$ by considering the effect of each of these two terms individually.
...
\end{align}
Since $\abs{S_{\perp}} = \sin \bar{\theta}_{\rm e}$, the instantaneous
oscillation amplitude $\abs{\Delta \bar{\theta}_{\rm e}}$ can be bound by:
\begin{equation}
...
$N$th Fourier harmonic, and the amplitude diverges for exact equality.
...
\begin{align}
    i\Phi(t) &\equiv \int\limits^t
        i \overline{\Omega}_{\rm e} + \Omega_{\rm eN}\cos (N\Omega_{\rm LK} t)
            \cos \p{\Delta I_{\rm eN}}\;\mathrm{d}t,\nonumber\\
...
evaluate $\mathcal{I}$ any integer multiple $m$ of its period $2\pi q$ to be
\begin{equation}
...
\end{equation}
We see that this differs from the conclusion of
Section~\ref{ss:intuitive_resonance} [Eq.~\eqref{eq:harmonic_dqeff1}], as the
$N$th Fourier harmonic generates infinitely many resonances indexed by $q \geq
1$.
...
which results in the same bound on the oscillation amplitude. Returning to
Eq.~\eqref{eq:formal_sol_terms}, we neglect the first, bounded term ($\lesssim
e^\eta \simeq 1$) on the right hand side and obtain the total oscillation
amplitude due to a $q$th order resonance with the $N$th Fourier harmonic
\begin{equation}
...
to the equation of motion containing all of the Fourier harmonics
[Eq.~\eqref{eq:formal_sol_gen}]. If $\overline{\Omega}_{\rm e} / \Omega_{\rm LK}
\approx M$, there can now be multiple $N$ and $q$ satisfying $Nq = M$. By
linearity, the total $\abs{\Delta \bar{\theta}_{\rm e}}$ is given by the sum
over all of the resonances, so that
\begin{equation}
    \abs{\Delta \bar{\theta}_{\rm e}} \approx \sum\limits_{N, q \mid Nq = M}
        \abs{\Delta \bar{\theta}_{\rm e}}_{Nq},\label{eq:dq_sum_res}
\end{equation}
where $\abs{\Delta \bar{\theta}_{\rm e}}_{Nq}$ is given by Eq.~\eqref{eq:dq_Nq}.
...
\begin{equation}
    \abs{\Delta \bar{\theta}_{\rm e}}_{Nq} \propto \frac{\cos^q\p{\Delta I_{\rm
        eN}} q^q}{2^qq!}.
\end{equation}
Stirling's approximation then suggests that $\abs{\Delta \bar{\theta}_{\rm
e}}_{Nq} \propto \p{\cos\p{\Delta I_{\rm eN}} e / 2}^q / \sqrt{q} \sim 1$. As
such, we conclude that all combinations of $N$ and $q$ satisfying $Nq = M$
result in comparable oscillation amplitudes. For simplicity, we evaluate this
amplitude for $q = 1$ and $N = M$. If we denote the number of pairs of $N$ and
$q$ satisfying $Nq = M$ by $d(M)$ (the number of positive divisors of $M$), we
can approximate Eq.~\eqref{eq:dq_sum_res} by:
\begin{align}
...
simulations when $M = 1$ or $M = 2$, the two relevant cases for the main text.
...
mergers (see Fig.~\ref{fig:dWs}), so we focus on the effect of the $M = 1$
...
the $M = 1$ resonance [Eq.~\eqref{eq:harmonic_dqeff}] in the bottom panel
...
Eq.~\eqref{eq:harmonic_dqeff} for $M = 1$ describes the oscillations in
...
dominant over that of the nonadiabatic contribution. In the previous section, we
showed that neglecting resonant terms still allows for an accurate prediction of
the final $\bar{\theta}_{\rm e}$ deviation [Eq.~\eqref{eq:prediction}]. This
implies that, while the resonances have a larger contribution to $\abs{\Delta
...
\begin{figure}
    ...
    where only systems with $I_0 \approx 90^\circ$ will merge within a
...
\begin{figure}
    ...
    = 3 \times 10^{7}M_{\odot}$, $a_0 = 0.1\;\mathrm{AU}$, $e_0 =
    ...
    simulations with $\theta_{\rm sl, f} = 0$ [these are symmetric about $I_0 =
    90^\circ$, as the equations of motion (\ref{eq:dadt}--\ref{eq:dwdt}) are as
    well]. The prediction for $\theta_{\rm sl, f}$ assuming conservation of
    ...
    substantial range of intermediate inclinations ($I_0 \in [50^\circ, 80^\circ]$
    and $I_0 \in [100^\circ, 130^\circ]$), $\theta_{\rm sl, f}$ is significantly
    affected by the resonances as they evolve outwards from $I = 90^\circ$ (see
    Fig.~\ref{fig:dWs_inner}). As such, these initial inclinations are expected
    to have uncorrelated $\theta_{\rm sl, f}$ and $\bar{\theta}_{\rm e, 0}$, and
    we denote this range with a broad red shaded region.}\label{fig:bin_comp}
\end{figure}
...
occurs ($I_0 \lesssim 50^\circ$ and $I_0 \gtrsim 130^\circ$) and
when $\abs{\mathcal{A}} \gg 1$ ($80^\circ \lesssim I_0 \lesssim
100^\circ$). However, we see in Fig.~\ref{fig:bin_comp} that for intermediate
inclinations $I_0 \in [50, 80]$ and $I_0 \in [100, 130]$,
$\theta_{\rm sl, f}$ varies over a large range and does not agree with the
prediction of $\bar{\theta}_{\rm e}$ conservation. Note that these inclinations
...
Fig.~\ref{fig:dWs_inner} illustrates that these intermediate inclinations
satisfy $\overline{\Omega}_{\rm e} \sim \Omega_{\rm LK}$. In addition, as the
inner binary coalesces under GW radiation, $e_{\min}$ becomes larger (e.g.\ see
Fig.~\ref{fig:4sim_90_350}). This causes the locations of the resonances to
evolve outwards from $I = 90^\circ$. As the location of the resonances is a
sensitive function of $e_{\min}$, resonance passage is nonadiabatic. Thus, we
expect that all systems that encounter the resonance, i.e.\ all systems with
intermediate initial inclinations, will experience an impulsive kick to
$\bar{\theta}_{\rm e}$, resulting in poor $\bar{\theta}_{\rm e}$ conservation.
This result is denoted by the broad red shaded regions in
Fig.~\ref{fig:bin_comp}.
...
$90^\circ$ start at the edge of the $M = 1$ resonance and quickly evolve out
from it (as $e_{\min}$ increases and $a$ decreases). As such, they only interact
briefly and weakly with the resonances, and the cumulative effect of the
resonance interaction can be estimated by evaluating
...
\begin{figure}
    ...
    regime. In the LK-enhanced regime, all initial inclinations merge within a
    Hubble time, and it is clear that $\overline{\Omega}_{\rm e} \approx
    \Omega_{\rm LK}$ can be satisfied for a wide range of initial
    inclinations.}\label{fig:dWs_inner}
\end{figure}
...
\begin{itemize}
    \item $\sin \p{\Delta I_{\rm eN}}$ is small unless $\mathcal{A} \simeq
        ...
        $\overline{\bv{\Omega}}_{\rm e}$ which implies that all the $\Delta
        I_{\rm eN} \approx 0$.
...
\begin{figure}
    ...
    $\overline{\Omega}_{\rm L} < 0$. Here, $\bv{L}_{\rm out}$ is not fixed, but
    $\bv{L}_{\rm tot} \equiv \bv{L} + \bv{L}_{\rm out}$ is. Note that the
    coordinate system is now oriented with $\uv{Z} \propto \bv{L}_{\rm
...
rate of precession of $\bv{L}$ about $\bv{L}_{\rm tot}$, and bars denote
...
while $\overline{\Omega}_{\rm L} \propto a^{3/2}$). When this is the case,
$\overline{\bv{\Omega}}_{\rm e} \parallel \bv{L}$, and so $\bar{\theta}_{\rm e,
...
First, we consider the case where $\bv{S}_0 \propto \bv{\overline{L}}_{\rm
...
$\overline{\Omega}_{\rm e}$ is either parallel or anti-parallel to $\bv{L}_{\rm
tot}$ depending on whether $\overline{\Omega}_{\rm L}$ is positive or negative.
We denote the starting mutual inclination for which $\overline{\Omega}_{\rm L}$
changes sign by $I_{\rm c}$. Note that $I_{\rm c} > 90^\circ$: even though
$\Omega_{\rm L}$ changes sign at $90^\circ$, the inclination decreases over a LK
cycle for $I < I_{\lim}$ (where $I_{\lim} > 90^\circ$ is the starting mutual
inclination that maximizes $e_{\max}$ \citep{bin2}), so the sign of $\Omega_{\rm
L}$ changes over a LK cycle for some $I_{\rm c} \in (90^\circ, I_{\lim})$. We
then obtain that
\begin{equation}
    \theta_{\rm sl, f} =
    \begin{cases}
...
neither of $\rdil{\ascnode}{t}$ and $\rdil{I_{\rm tot}}{t}$ are zero at $I_{\rm
0} = I_{\rm c}$.

Eq.~\eqref{eq:qslf_eta_pred} also gives good agreement in LK-enhanced menger
...
The systems detected by LIGO in O1 and O2 have $\chi_{\rm eff} \approx 0$, where
$\chi_{\rm eff}$ is given by Eq.~\eqref{eq:chi_eff}
\citep{Abbott:2016blz, abbott2019binary}. One possible explanation for
...
case, $\bv{\Omega}_{\rm e}$ has neither consistent direction nor magnitude, and
