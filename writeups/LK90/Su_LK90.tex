...
\title[BH Triple Spin Dynamics]{Spin Dynamics in Hierarchical Black Hole
Triples: Predicting Final Spin-Orbit Misalignment Angle From Initial Conditions}
\author[Y. Su et\ al.]{
Yubo Su,$^1$,
Dong Lai$^1$
Bin Liu$^1$
...
\date{Accepted Jan 31, 2999. Received Jan 31, 2999; in original form Jan 31,
2999}
...
\begin{abstract}
    Many previous studies have considered the spin and orbital dynamics of
    black hole (BH) binaries that undergo large eccentricity excitations through
    Lidov-Kozai oscillations due to the influence of a highly inclined tertiary
    companion and merge via gravitational wave radiation. However, these extreme
    eccentricity variations make such systems difficult to characterize
    analytically. In this paper, we provide an analytical formalism for
    understanding the spin dynamics of the inner BHs to quadrupole order. We
    show that, under certain conditions, the regular eccentricity oscillations
    of the binary can be averaged over to determine the long-term behavior of
    the system in a smooth and gradual way. In particular, we demonstrate that
    the final spin-orbit misalignment angle when the inner binary
    gravitationally decouples from the tertiary companion is often simply
    related to the binary's primordial spin orientation through an approximate
    adiabatic invariant. We calculate the deviation from adiabaticity in closed
    form as a function of initial conditions. We also place accurate constraints
    on when this adiabatic invariant breaks down due to resonant interactions.
    We consider both stellar-mass and supermassive BH tertiary companions, and
    provide simple prescriptions for astrophysically relevant parameters.
\end{abstract}
...
The recently-announced LIGO detection GW190521 \citep{190521} has $\chi_{\rm
eff} = 0.08_{-0.38}^{+0.27}$ while $\chi_{\rm p} = 0.68_{-0.37}^{+0.25}$. The
spin-orbit misalignment angles of the two merging BHs are reported to be
$81_{-53}^{+64}$ and $85_{-55}^{+57}$ degrees respectively.
...
which an initially wide BH binary (too wide to merge in isolation) is pushed to
extreme eccentricities (close to unity) by a highly inclined tertiary and merges
within a few Gyrs. LL18 examined a wide range of orbital and spin evolution
behaviors and found that LK-induced mergers can sometimes yield a ``$90^\circ$
attractor'': when the BH spin is initially aligned with the inner binary angular
momentum axis ($\theta_{\rm sl, 0} = 0$), it evolves towards a perpendicular
state ($\theta_{\rm sl, f} = 90^\circ$) near merger. Qualitatively, they found
that the attractor exists when the LK-induced orbital decay is sufficiently
``gentle'' and the octupole effect is unimportant. Fig.~\ref{fig:4sim_90_350}
gives an example of a system evolving towards this attractor, where $\theta_{\rm
sl}$ converges to $\approx 90^\circ$ at late times in the bottom right panel.
Fig.~\ref{fig:qslscan} shows how $\theta_{\rm sl, f}$ varies when the initial
inclination of the tertiary orbit $I_{\rm 0}$ (relative to the inner orbit) is
varied. Note that for rapid mergers (when $I_{\rm 0}$ is close to $90^\circ$),
the attractor does not exist; as $I_{\rm 0}$ deviates more from $90^\circ$, the
merger time increases and $\theta_{\rm sl, f}$ is close to $90^\circ$.  This
$90^\circ$ attractor gives rise to a peak around $\chi_{\rm eff} = 0$ in the
final $\chi_{\rm eff}$ distribution in tertiary-induced mergers (LL18;
...
\begin{figure}
    ...
    $a_{\rm 0} = 100\;\mathrm{AU}$, $e_{\rm 0} = 0.001$, $I_{\rm 0} =
    90.35^\circ$ (with respect to the outer binary), and $\theta_{\rm sl, 0} =
    0$. The tertiary SMBH has $a_{\rm out} = 2.2\;\mathrm{pc}$, $e_{\rm out} =
    0$, and $m_3 = 3 \times 10^7 M_{\odot}$. It can be seen that $\theta_{\rm
    sl}$ evolves to $\sim 90^\circ$ as $a$ decays to smaller values, and we stop
    the simulation when $a = 0.5\;\mathrm{AU}$ as $\theta_{\rm sl}$ has
    converged.}\label{fig:4sim_90_350}
\end{figure}
\begin{figure}
    \centering
    ...
    $\theta_{\rm sl, f}$ as a function of the initial inclination $I_{\rm 0}$
    for LK-induced mergers. The other parameters are the same as those in
    Fig.~\ref{fig:4sim_90_350}. For $I_{\rm 0}$ somewhat far away from
    $90^\circ$, the resulting $\theta_{\rm sl, f}$ are all quite near
    $90^\circ$. The horizontal black solid line shows the
    predicted $\theta_{\rm sl, f}$ if $\bar{\theta}_{\rm e}$ is conserved, i.e.\
    Eq.~\eqref{eq:4_4_general}. The black dashed line in the lower panel shows
...
be easily generalized to comparable masses. We denote the orbital angular
momentum of the inner binary by $\bm{L} \equiv L \uv{L}$ and the angular
momentum of the outer binary by $\bm{L}_{\rm out} \equiv L_{\rm out} \uv{L}_{\rm
out}$. Since $L_{\rm out} \gg L$, we take $\bm{L}_{\rm out}$ to be fixed.
...
our fiducial parameters (described in the figure caption). In this example, the
inner binary initially coalesces slowly as regular LK oscillations excite the
eccentricity to a consistent maximum near unity. Then, as $a$ decreases and the
minimum eccentricity within each LK cycle increases, the orbital decay of the
inner binary accelerates. Once the binary becomes sufficiently compact, the
spin-orbit precession timescale is very short, and $\theta_{\rm sl}$ reaches a
constant value $\approx 90^\circ$. At the same time, apsidal precession
suppresses further eccentricity oscillations, and the eccentricity begins to
rapidly decay to $0$. We terminate the simulation at $a = 0.5\;\mathrm{AU}$ as
$\theta_{\rm sl}$ has reached its final value, even though the system still has
substantially nonzero eccentricity. Since $\tilde{a}_{\rm out} \gg a$, the
octupole effects are negligible and have been omitted in
Eqs.~(\ref{eq:dedt}--\ref{eq:dwdt}). Note that these parameters give the same
$t_{\rm LK}$ as Fig.~4 of LL18. We refer to this as the fiducial parameter
regime, and our analysis in later sections will be based on this example unless
otherwise noted.
...
\end{align}
(see \citealp{anderson2016formation}, LL18 for more general expressions
...
$e_{\max}$ as a function of the initial $I_{\rm 0}$ (and initial $e_{\rm 0} \ll 1$).
The largest value of $e_{\max}$ occurs at $I_{\rm 0} = 90^\circ$ and is given
by
\begin{equation}
    j(e_{\max})_{I_{\rm 0} = 90^\circ} = (8/9) \epsilon_{\rm GR}.
        \label{eq:jemaxepsgr}
\end{equation}
...
$\epsilon_{\rm GR} \ll 1$ at $a = a_{\rm 0}$, leading to $e_{\max} \sim 1$ within a
narrow inclination window around $I_{\rm 0} = 90^\circ$.
...
\subsection{Nondissipative Dynamics}\label{ss:monodromy}
...
\begin{equation}
    \p{\rd{\bm{S}}{t}}_{\rm rot}
        = \s{\overline{\bm{\Omega}}_{\rm e} + \sum\limits_{N = 1}^\infty
            \bm{\Omega}_{\rm eN}\cos \p{N\Omega_{\rm LK}t}}
            \times \bm{S}.\label{eq:dsdt_fullft}
\end{equation}
Note that $\overline{\bm{\Omega}}_{\rm e}$ is the zeroth Fourier component,
...
0.1\;\mathrm{AU}$, $e_{\rm 0} = 10^{-3}$, $I_{\rm 0} = 70^\circ$, $a_{\rm out} =
...
\begin{equation}
    \cos \theta_{\rm e} = \frac{\overline{\bm{\Omega}}_{\rm e}}{
...
\begin{figure}
    ...
    \times 10^7 M_{\odot}$, $a = 0.1\;\mathrm{AU}$, $e_{\rm 0} = 10^{-3}$,
    $I_{\rm 0} = 70^\circ$, $a_{\rm out} = 300\;\mathrm{AU}$, $e_{\rm out} = 0$,
    ...
    $\theta_{\rm sl}$ (the angle between $\bm{S}$ and $\uv{L}$) and $\theta_{\rm
...
For our system, while $\bm{\tilde{M}}$ can be easily defined, it cannot be
evaluated in closed form. Instead, we can reason directly about the general
properties of $\bm{\tilde{M}}$: it must be a proper orthogonal matrix, or a
rotation matrix, as it represents the effect of many infinitesimal rotations,
each about the instantaneous $\bm{\Omega}_{\rm e}$\footnote{More formally,
$\bm{\tilde{M}} = \bm{\tilde{\Phi}}(P_{\rm LK})$ where $\bm{\tilde{\Phi}}(t)$ is
the \emph{principal fundamental matrix solution}: the columns of
$\bm{\tilde{\Phi}}$ are solutions to Eq.~\eqref{eq:dsdt_weff} and
...
matrix.}. Therefore, over each period $P_{\rm LK}$, the dynamics of $\bm{S}$ are
...
$\overline{\bm{\Omega}}_{\rm e}$ [see Eq.~\eqref{eq:dsdt_fullft}], are no longer
...
the binary goes through a sequence of LK cycles, and the coefficents vary on the
LK-averaged orbital decay time $t_{\rm GW}\p{e_{\max}} / j\p{e_{\max}}$. As the
LK oscillation freezes, we have $\bm{\Omega}_{\rm e} \simeq
\overline{\bm{\Omega}}_{\rm e}$ (and $\bm{\Omega}_{\rm eN} \simeq 0$ for $N \geq
1$), which evolves on timesale $t_{\rm GW}(e)$ as the orbit decays and
circularizes.
...
angle between $\overline{\bm{\Omega}}_{\rm e}$ and $\bm{L}_{\rm out}$ as shown in
...
\begin{figure}
    ...
    Eq.~\eqref{eq:barI}, $\overline{\bm{\Omega}}_{\rm e}$ is the LK-averaged
    $\bm{\Omega}_{\rm e}$, and $\bm{\Omega}_{\rm e1}$ is the first harmonic
    component (see Eqs.~\eqref{eq:weff_def} and~\eqref{eq:dsdt_fullft}). Note
    that for $I_{\rm 0} > 90^\circ$ (and $\bar{I} > 90^\circ$), we choose
...
\overline{\bm{\Omega}}_{\rm e}$ and $\uv{x}$ lies in the plane of $\bm{L}_{\rm
...
and focus on how the varying $\overline{\bm{\Omega}}_{\rm e}$ affects the
...
Eq.~\eqref{eq:dsdt_0only}, motivating our notation $\overline{\bm{S}}$. Over
...
\begin{equation}
    \cos \bar{\theta}_{\rm e} \equiv
        \frac{\overline{\bm{\Omega}}_{\rm e}}{\overline{\Omega}_{\rm e}}
            \cdot \overline{\bm{S}},
        \label{eq:q_eff}
\end{equation}
...
$\abs{\dot{\bar{I}}_{\rm e}} \ll \overline{\Omega}_{\rm e}$ at all times.
...
\begin{equation}
    \Phi(t) \equiv \int\limits^t \overline{\Omega}_{\rm e}\;\mathrm{d}t,
        \label{eq:Phi_t}
\end{equation}
we obtain the solution between the initial time $t_{\rm 0}$ and the final time
$t_{\rm f}$:
\begin{equation}
    e^{-i\Phi}S_{\perp}\bigg|_{t_{\rm 0}}^{t_{\rm f}}
        = -\int\limits_{t_{\rm 0}}^{t_{\rm f}}
...
in $\bar{\theta}_{\rm e}$ over $t \in [t_{\rm 0}, t_{\rm f}]$, we can give a
loose bound
\begin{equation}
...
binary merger starting with $I_{\rm 0} = 90.2^\circ$, for which $\abs{\Delta
...
\begin{figure}
    ...
    conservation of $\bar{\theta}_{\rm e}$ due to nonadiabaticity [green,
    Eq.~\eqref{eq:nonad_dqeff}] and due to resonances with the $N = 1$ harmonic
    [blue, Eq.~\eqref{eq:harmonic_dqeff}].}\label{fig:4sim_90_350_supp}
\end{figure}
\begin{figure*}
    \centering
...
\begin{figure*}
    ...
    \caption{Same as Fig.~\ref{fig:4sim_90_350_zoom} except for $I_{\rm 0} =
...
\begin{equation}
    j_{\star} \equiv j(e_{\star}) = f
        \sqrt{\frac{5}{3}\cos^2 I_{\rm 0}},\label{eq:jstar_ansatz}
\end{equation}
where the star subscript denotes evaluation at $\mathcal{A} \simeq 1$ and $f>1$
is a constant. Eq.~\eqref{eq:jstar_ansatz} assumes that $I_{\rm 0}$ far enough from
$90^\circ$ that the GR effect is unimportant in determining $e_{\max}$. The
value of f turns out to be relatively insensitive to $I_{\rm 0}$.
...
\begin{align}
    \abs{\frac{\dot{\bar{I}}_{\rm e}}{\overline{\Omega}_{\rm e}}}_{\max}
        ...
        \left[\frac{8000 G^{9} m_{12}^{9} m_3^{3}
            ...
            3)^{11}}\right]^{1/8}.
\end{align}
We can also calculate $\abs{\dot{\bar{I}}_{\rm e}} / \overline{\Omega}_{\rm e}$ from
...
holds across a range of $I_{\rm 0}$), we fit the last remaining free parameter $f$
[Eq.~\eqref{eq:jstar_ansatz}] to the data from numerical simulations. This
yields $f \approx 2.72$, leading to
\begin{align}
    \abs{\frac{\dot{\bar{I}}_{\rm e}}{\overline{\Omega}_{\rm e}}}_{\max}
        \simeq{}& 0.98^\circ \p{\frac{\cos I_{\rm 0}}{\cos (90.3^\circ)}}^{-37/8}
...
\begin{figure}
    ...
    start of inspiral $t_{\rm LK, 0}$. The agreement between the analytical
    and numerical results is excellent for $T_{\rm m} \gg t_{\rm
...
computing the quotient $\abs{\dot{\bar{I}}_{\rm e}} / \overline{\Omega}_{\rm
e}$.
...
carry out simulations for a range of $I_{\rm 0}$, and for each $I_{\rm 0}$ we consider $100$
...
\begin{figure}
    ...
    function of initial inclination $I_{\rm 0}$. For each $I_{\rm 0}$, $100$ simulations are
...
$\bm{S}$ and $\overline{\bm{\Omega}}_{\rm e}$ is conserved between $t = t_{\rm
0}$ and $t = t_{\rm f}$, i.e.
\begin{equation}
    \bar{\theta}_{\rm e, f} \simeq \bar{\theta}_{\rm e, 0}.
        \label{eq:4_4_qeffcons}
\end{equation}
At $t = t_f$, $\overline{\Omega}_{\rm SL} \gg \abs{\overline{\Omega}_{\rm L}}$
and the spin-orbit misalignment angle $\theta_{\rm sl}$ is ``frozen'', implying
$\overline{\bm{\Omega}}_{\rm e}$ is parallel to $\bm{L}$, and so
$\bar{\theta}_{\rm e, f} = \theta_{\rm sl, f}$.
Eq.~\eqref{eq:4_4_qeffcons} then gives
\begin{equation}
...
$\bm{S}$ and $\overline{\bm{\Omega}}_{\rm e}$.

Now, first consider the case where the initial spin $\bm{S}_{\rm 0}$ is aligned
with the initial $\bm{L}_{\rm 0}$. This initial spin is inclined with respect to
$\overline{\bm{\Omega}}_{\rm e}$ by $\bar{\theta}_{\rm e, 0} = \abs{I_{\rm 0} -
\bar{I}_{\rm e, 0}}$, where $I_{\rm 0}$ is the initial inclination angle
between $\bm{L}$ and $\bm{L}_{\rm out}$ and $\bar{I}_{\rm e, 0}$ is the
initial value of $\bar{I}_{\rm e}$. Thus, adiabatic invariance implies
\begin{equation}
    \theta_{\rm sl, f} \simeq \abs{I_{\rm 0} - \bar{I}_{\rm e, 0}}.
\end{equation}
In the special case where the binary initially satisfies $\overline{\Omega}_{\rm
SL} \ll \abs{\overline{\Omega}_{\rm L}}$ or $\abs{\mathcal{A}_{\rm 0}} \ll 1$,
we find that $\overline{\bm{\Omega}}_{\rm e}$ is nearly parallel to $\bm{L}_{\rm
out}$ (for $I_{\rm 0} < 90^\circ$) or antiparallel to $\bm{L}_{\rm out}$ (for
$I_{\rm 0} > 90^\circ$). Thus,
\begin{equation}
    \theta_{\rm sl, f} =
    \begin{cases}
        I_{\rm 0} & I_{\rm 0} < 90^\circ,\\
        180^\circ - I_{\rm 0} & I_{\rm 0} > 90^\circ.
    \end{cases}
\end{equation}
Since LK-induced mergers necessarily require $I_{\rm 0}$ close to $90^\circ$, we
find that $\theta_{\rm sl, f}$ is ``attracted'' to $90^\circ$.

Eq.~\eqref{eq:4_4_general} can be applied to more general initial spin
orientations. For initial $\abs{\mathcal{A}_{\rm 0}} \ll 1$ (as required for
LK-induced mergers), $\overline{\Omega}_{\rm e, 0}$ is parallel to $\pm
\bm{L}_{\rm out}$. Suppose the initial inclination between $\bm{S}$ and
$\bm{L}_{\rm out}$ is $\theta_{\rm s,out, 0}$, then $\bar{\theta}_{\rm
e, 0} = \theta_{\rm s,out, 0}$ or $180^\circ - \theta_{\rm s,
out, 0}$ (depending on whether $I_{\rm 0} < 90^\circ$ or $I_{\rm 0} >
90^\circ$). Thus,
\begin{equation}
    \theta_{\rm sl, f} \simeq
    \begin{cases}
        \theta_{\rm s,out, 0} & I_{\rm 0} < 90^\circ,\\
        180^\circ - \theta_{\rm s,out, 0} & I_{\rm 0} > 90^\circ,
    \end{cases}
\end{equation}
...
$\theta_{\rm sl, f}$ from $\sim 90^\circ$ except very near $I_{\rm 0} =
90^\circ$. This is expected, as Eq.~\eqref{eq:prediction} is not very accurate
very near $I_{\rm 0} = 90^\circ$, where it diverges (see Fig.~\ref{fig:deviations}).

When $I_{\rm 0} = 90^\circ$ exactly, Fig.~\ref{fig:qslscan} shows that $\theta_{\rm
sl, f} = 0^\circ$. This can be understood: $I_{\rm 0} = 90^\circ$ gives
$\rdil{I}{t} = 0$ by Eq.~\eqref{eq:dIdt}, so $I = 90^\circ$ for all time. This
then implies $\rdil{\ascnode}{t} = 0$ [Eq.~\eqref{eq:dWdt}], implying that
$\bm{L}$ is constant. Thus, $\bm{L}$ is fixed as $\bm{S}$ precesses around it,
and $\theta_{\rm sl}$ can never change. In Fig.~\ref{fig:qslscan}, we take
$\theta_{\rm sl, 0} = 0$, so $\theta_{\rm sl, f} = 0$.

Finally, Fig.~\ref{fig:qslscan} shows that the actual $\theta_{\rm sl, f}$
...
in Eq.~\eqref{eq:formal_sol_0}, denoted $\Phi(t_{\rm 0})$ and $\Phi(t_{\rm f})$.
When $\theta_{\rm sl, 0} = 0$, we have $\Phi(t_{\rm 0}) = 0$, as $\bm{S}$
starts in the $\uv{x}$-$\uv{z}$ plane. Then, as $I_{\rm 0}$ is smoothly varied, the
...
\bar{\theta}_{\rm e}}_{\rm f}$ as $I_{\rm 0}$ is changed, giving rise to the
...
% orbital evolution), different $\bm{S}_{\rm 0}$ with the same $\Phi(t_{\rm 0})$
...
In the previous section, we showed that the $\theta_{\rm sl, f}$ distribution
can be well understood when assuming the $N \geq 1$ Fourier harmonics in
Eq.~\eqref{eq:dsdt_fullft} can be neglected. In this section, we study the
effects of these neglected terms and their effect in the LK-induced and
LK-enhanced regimes.
...
\subsection{Near-Resonance Oscillation Amplitude}\label{ss:resonance}

We first restrict our analysis to a single Fourier harmonic at a time, indexed
by $N$. Eq.~\eqref{eq:formal_sol_gen} further simplifies to
\begin{align}
    \rd{S_{\perp}}{t} ={}& i\overline{\Omega}_{\rm e}S_\perp
        + \Big[
            \cos \p{\Delta I_{\rm eN}}S_\perp \nonumber\\
        &- i\cos \bar{\theta}_{\rm e} \sin \p{\Delta I_{\rm eN}}\Big]
            \Omega_{\rm eN}\cos (N\Omega_{\rm LK} t).\label{eq:formal_sol_gen1}
\end{align}

The general solution to this is given by
\begin{align}
    \s{e^{-i\Phi} S_\perp}_{t_{\rm 0}}^{t} &=
        -i \cos \bar{\theta}_{\rm e} \sin \Delta I_{\rm eN}
        \int\limits_{t_{\rm 0}}^{t}
            \Omega_{\rm eN}
            \cos\p{N \Omega \tau}e^{-i\Phi(\tau)} \;\mathrm{d}\tau,\nonumber\\
        &= iA\s{e^{-i\Phi}\Bigg|_{t_{\rm 0}}^{t}
            + i\overline{\Omega}_{\rm e}\int\limits_{t_{\rm 0}}^t
...
\begin{align}
    i\Phi(t) &\equiv \int\limits^t
        i\overline{\Omega}_{\rm e} + \cos (N\Omega_{\rm LK} t)
            \cos \p{\Delta I_{\rm eN}} \Omega_{\rm eN}\;\mathrm{d}t,\nonumber\\
        &\equiv i\overline{\Omega}_{\rm e}t + \eta \sin\p{N\Omega_{\rm LK} t},
\end{align}
where $\eta \equiv \p{\cos \Delta I_{\rm eN} \Omega_{\rm eN}}/\p{N\Omega_{\rm
LK}}$. Eq.~\eqref{eq:formal_sol_terms} shows that $\abs{S_{\perp}}$ is bounded
unless the integral $\mathcal{I}(x)$, given by
\begin{equation}
    \mathcal{I}(x) = \int\limits_0^x e^{-i\xi - \eta \sin\p{\beta
    t}}\;\mathrm{d}\xi,
\end{equation}
grows without bound as $x \to \infty$, where we have rescaled $x =
\overline{\Omega}_{\rm e}\p{t - t_{\rm 0}}$ and $\beta = N\Omega_{\rm LK} /
...
$\cos \xi$ or $\sin \xi$ term. Upon inspection, $\sin^{k}\p{\beta \xi}$ only
contains a term with unit frequency if $\beta = 1 / q$ for some integer $q$ and
$k \geq q$. When this is the case, we can evaluate $\mathcal{I}(x)$ over one
period $x = 2\pi q$ and obtain
\begin{equation}
    \abs{\mathcal{I}\p{2\pi q}} = 2\pi q \p{\frac{\eta^q}{2^q q!}
        + \mathcal{O}\p{\frac{1}{(q + 1)!}}}.
\end{equation}
The higher order corrections arise from the $k \geq q + 1$ terms in
Eq.~\eqref{eq:I_sum}. Since every period has a fixed, nonzero contribution to
$\mathcal{I}(x)$, $\abs{\mathcal{I}(x)}$ grows without bound when
\begin{equation}
...
\ll 1$, we can show that the amplitude of oscillation of $\abs{\mathcal{I}(x)}$
is bounded but can become large. If $q$ is even, then the dominant behavior is
\begin{align}
    \abs{\mathcal{I}\p{x}} \simeq{}& \int\limits_0^{x}
        \cos \xi \frac{\abs{\eta}^q}{q!}\frac{1}{2^{q - 1}}
            \cos \p{q \beta \xi}\;\mathrm{d}\xi,\nonumber\\
        \simeq{}& \frac{\abs{\eta}^q}{q!}\frac{1}{2^{q - 1}}
            \int\limits_0^{x} \frac{1}{2}
                \Big[\cos\p{\xi\p{1 + q\beta}}
            + \cos\p{\xi\p{1 - q\beta}}\Big]\;\mathrm{d}\xi,\nonumber\\
        \lesssim& \frac{\abs{\eta}^q}{2^qq!}\frac{1}{\abs{1 -
        q\beta}}.\label{eq:scaling}
\end{align}
If $q$ is odd, we use Eq.~\eqref{eq:identity2} instead of
Eq.~\eqref{eq:identity1} and integrate against $i\sin x$ instead of $\cos x$,
which results in the same bound on the oscillation amplitude. Thus, we see that
if $0 < \abs{1 - q\beta} \ll 1$, i.e.\ the system is near but not on resonance,
$S_{\perp}$ oscillates over a large but finite range.

\subsection{Effect of Multiple Resonances}

In the previous section, we considered only the effect of a single $N$. However,
if $\overline{\Omega}_{\rm e} / \Omega_{\rm LK} \approx M$ where $M$ is an
integer greater than $1$, there are multiple combinations of $N$ and $q$ that
satisfy $M = Nq$. We can attempt to understand whether particular combinations
of $N$ and $q$ dominate the response in $S_{\perp}$. We make a few simplifying
assumptions: (i) all $N$ harmonics are approximately equal\footnote{This
approximation is suitable for the problem studied in the main text because the
only characteristic frequency scale is $j_{\min}^{-1} \gg 1$, so all Fourier
harmonics $\bm{\Omega}_{\rm eN}$ for $N \lesssim j_{\min}^{-1}$ are similar.},
(ii) $\Omega_{\rm eN} \sim \overline{\Omega}_{\rm e}$ and $\cos \Delta I_{\rm
eN} \simeq 1$ (e.g.\ Figs.~\ref{fig:4sim_90_350_supp}
and~\ref{fig:4sim_90_350_zoom}), and (iii) the oscillation amplitude of
$S_{\perp}$ due to multiple resonances is the sum of the amplitude due to each
individual resonance. Under the first two assumptions, Eq.~\eqref{eq:scaling}
scales like:
\begin{equation}
    \max \abs{\mathcal{I}(x)} \propto \frac{\cos^q\p{\Delta I_{\rm eN}}
    q^q}{2^qq!}.
\end{equation}
For $q = 1$ and $q = 2$, this expression changes by a factor of $\cos \Delta
I_{\rm eN} \simeq 1$. For general $q$, Stirling's approximation suggests that
$\max \abs{\mathcal{I}(x)} \approx \p{\cos\p{\Delta I_{\rm eN}} e / 2}^q \sim
1$. As such, we conclude that all combinations of $N$ and $q$ satisfying $Nq =
M$ result in comparable oscillation amplitudes. Denote the number of such pairs
of $N$ and $q$ by $d(M)$, which is just the number of positive divisors of $M$.
Then, by our third assumption above, the total oscillation amplitude is just the
sum of these contributions, which we evaluate at $q = 1, N = M$ for simplicity:
\begin{equation}
    \abs{\s{e^{-i\Phi}S_{\perp}}_{t_{\rm 0}}^t} \lesssim
        \frac{d(M)}{2} \abs{\frac{A\eta}{\overline{\Omega}_{\rm e} -
        M\Omega_{\rm LK}}}.
\end{equation}

Finally, we conclude that the effect of all the Fourier harmonics together
causes $\bar{\theta}_{\rm e} = \sqrt{1 - \abs{S_{\perp}}^2}$ to oscillate with
semi-amplitude
\begin{equation}
    \abs{\Delta \bar{\theta}_{\rm e}} \sim \frac{d(N)}{2}
        \abs{\frac{\sin \Delta I_{\rm eN}\Omega_{\rm eN}}
        {\overline{\Omega}_{\rm e} - N\Omega_{\rm LK}}},
        \label{eq:harmonic_dqeff}
\end{equation}
where we have set $M = N$ as $q = 1$. Note that $d(1) = 1$ while $d(2) = 2$.
While we have made some crude approximations in deriving
Eq.~\ref{eq:harmonic_dqeff}, Appendix~\ref{app:numeric} demonstrates
that it is in good agreement with detailed numerical simulations when $N = 1$ or
$N = 2$, the two relevant cases for the main text.

\subsection{Effect of Resonances in LK-Induced Mergers}

We first consider the effect of these resonances on the LK-induced regime, using
the fiducial parameters. Numerically, we find that $\overline{\Omega}_{\rm e} <
\Omega_{\rm LK}$ for the region of parameter space relevant to LK-induced
mergers (see Fig.~\ref{fig:dWs}), so we focus on the effect of the $N = 1$
resonance. If, for the entire inspiral, $\overline{\Omega}_{\rm e} < \Omega_{\rm
LK}$ by a sufficient margin that Eq.~\eqref{eq:harmonic_dqeff} remains small,
then the conservation of $\bar{\theta}_{\rm e}$ cannot be significantly affected
by this resonance. For the fiducial simulation, Fig.~\ref{fig:4sim_90_350_supp}
shows the ratio $\overline{\Omega}_{\rm e} / \Omega_{\rm LK}$ in the middle
panel (black) and the amplitude of oscillation of $\bar{\theta}_{\rm e}$ due to
the $N = 1$ resonance [Eq.~\eqref{eq:harmonic_dqeff}] in the bottom panel
(blue). We see that the system is never close to the resonant condition
$\overline{\Omega}_{\rm e} / \Omega_{\rm LK} = 1$, and as a result the total
effect of the resonance never exceeds a few degrees.

For a more precise comparison, the bottom-rightmost panel of
Fig.~\ref{fig:4sim_90_350_zoom} compares $\abs{\Delta \bar{\theta}_{\rm e}}$ in
the fiducial simulation to the expected contributions from nonadiabatic
[Eq.~\eqref{eq:nonad_dqeff}] and resonant [Eq.~\eqref{eq:harmonic_dqeff} for $N
= 1$] effects in the regime where $\mathcal{A} \simeq 1$. We see that
Eq.~\eqref{eq:harmonic_dqeff} for $N = 1$ describes the oscillations in
$\bar{\theta}_{\rm e}$ very well. The agreement is poorer in the
bottom-rightmost panel of Fig.~\ref{fig:4sim_90_200_zoom}, as the nonadiabatic
effect is much stronger.

It is surprising that the contribution of the resonant terms to the
instantaneous $\Delta \bar{\theta}_{\rm e}$ in the $\mathcal{A} \simeq 1$ regime
is dominant over that of the nonadiabatic contribution. In the previous section,
we showed that neglecting resonant terms still allows for an accurate prediction
of the final $\bar{\theta}_{\rm e}$ deviation [Eq.~\eqref{eq:prediction}]. This
implies that the nonadiabatic effect is instead more important than the resonant
terms in determining the final deviation. This also requires that a $\abs{\Delta
\bar{\theta}_{\rm e}}$ of up to a few degrees due to the $N \geq 1$ Fourier
harmonics not affect the final nonconservation of $\bar{\theta}_{\rm e}$ by more
than $\sim 0.01^\circ$. This differs from the nonadiabatic case, where we find
$\abs{\Delta \bar{\theta}_{\rm e}}_{\rm f} \sim \max \abs{\Delta
\bar{\theta}_{\rm e}}$. We do not have an explanation for this discrepancy.
...
\begin{figure}
    ...
    for the fiducial parameter regime. In the fiducial parameter regime, where
    where only systems with $I_{\rm 0} \approx 90^\circ$ will merge within a
    Hubble time, $I(e_{\min}) \sim 90^\circ$ for most of the evolution (see
    Fig.~\ref{fig:4sim_90_350}) until $e_{\min} \approx 1$ is satisfied. This
    plot then shows that $\overline{\Omega}_{\rm e} \lesssim 0.5 \Omega_{\rm
    LK}$ is a general feature of LK-induced mergers, as is the case for the
    fiducial simulation (see Fig.~\ref{fig:4sim_90_350_supp}).}\label{fig:dWs}
\end{figure}
...
SMBH\@: $m_1 = m_2 = 30M_{\odot}$, $a_{\rm 0} = 0.1\;\mathrm{AU}$, $e_{\rm 0} =
10^{-3}$, $m_3 = 3 \times 10^7 M_{\odot}$, $\tilde{a}_{\rm out} =
300\;\mathrm{AU}$, and $e_{\rm out} = 0$. We show that the resonances studied
above play an important role in shaping the $\theta_{\rm sl, f}$ distribution in
this regime.

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{LK90_plots/8bin_comp_eta0.png}
    \caption{The merger time (top), the magnitude of the adiabaticity parameter
    $\abs{\mathcal{A}} \equiv \Omega_{\rm SL} / \abs{\Omega_{\rm L}}$ (middle),
    and the final spin-orbit misalignment angle $\theta_{\rm sl, f}$ (bottom)
    for the LK-enhanced parameter regime, i.e.\ $m_1 = m_2 = 30M_{\odot}$, $m_3
    = 3 \times 10^{7}M_{\odot}$, $a_{\rm 0} = 0.1\;\mathrm{AU}$, $e_{\rm 0} =
    10^{-3}$, $\tilde{a}_{\rm out} = 300\;\mathrm{AU}$, and $e_{\rm out} = 0$.
    In the middle panel, the horizontal dashed line indicates $\abs{\mathcal{A}}
    = 1$. On the bottom panel, the blue dots denote results from numerical
    simulations with $\theta_{\rm sl, f} = 0$. The prediction for $\theta_{\rm
    sl, f}$ assuming conservation of $\bar{\theta}_{\rm e}$ is shown as the red
    line, which agrees well with the data both when there is no eccentricity
    excitation ($I \lesssim 50^\circ$ and $I \gtrsim 130^\circ$) and when
    $\abs{\mathcal{A}} \gg 1$. The red shaded area shows the predicted deviation
    from conservation of $\theta_{\rm e}$, given by
    Eq.~\eqref{eq:harmonic_dqeff} for $N = 1$. For a substantial range of
    intermediate inclinations ($I \in [50^\circ, 80^\circ]$ and $I \in
    [100^\circ, 130^\circ]$), the spin is significantly affected the resonances
    as they sweep away from $I = 90^\circ$ (see Fig.~\ref{fig:dWs_inner}).
    As such, these initial inclinations are expected to have uncorrelated
    $\theta_{\rm sl, f}$ and $\theta_{\rm e, 0}$, and we denote this range with
    a broad red shaded interval.}\label{fig:bin_comp}
\end{figure}
...
prediction assuming conservation of $\bar{\theta}_{\rm e}$ is shown in the red
solid line. Good agreement is observed both when no eccentricity excitation
occurs ($I_{\rm 0} \lesssim 50^\circ$ and $I_{\rm 0} \gtrsim 130^\circ$) and
when $\abs{\mathcal{A}} \gg 1$ ($80^\circ \lesssim I_{\rm 0} \lesssim
100^\circ$). However, we see in Fig.~\ref{fig:bin_comp} that for intermediate
inclinations $I_{\rm 0} \in [50, 80]$ and $I_{\rm 0} \in [100, 130]$,
$\theta_{\rm sl, f}$ varies over a large range and does not agree with the
prediction of $\bar{\theta}_{\rm e}$ conservation. This pattern of deviations is
different than that seen in the fiducial parameter regime
(Fig.~\ref{fig:qslscan}), as this inclination regime corresponds to neither the
fastest nor slowest merging systems.

We attribute the origin of these deviations to resonance interactions.
Fig.~\ref{fig:dWs_inner} illustrates that these intermediate inclinations are
closer to the $\overline{\Omega}_{\rm e} = \Omega_{\rm LK}$ resonance than
inclinations very close to $90^\circ$. In addition, as the inner binary
coalesces under GW radiation, $e_{\min}$ becomes larger (e.g.\ see
Fig.~\ref{fig:4sim_90_350}), so the locations of the resonances sweep away from
$I = 90^\circ$. As the location of the resonances is a sensitive function of
$e_{\min}$, resonance passage is likely nonadiabatic. Thus, we expect that all
systems that encounter the resonance, i.e.\ all systems with intermediate
initial inclinations, will experience an impulsive kick to $\bar{\theta}_{\rm
e}$, resulting in poor $\bar{\theta}_{\rm e}$ conservation. This result is
denoted by the broad red shaded regions in Fig.~\ref{fig:bin_comp}.

For initial inclinations closer to $90^\circ$, these systems start at the edge
of the $N = 1$ resonance and quickly evolve away from it (as $e_{\min}$
increases and causes $\overline{\Omega}_{\rm e} / \Omega_{\rm LK}$ to decrease),
so the cumulative effect of the resonance interaction is well predicted by
evaluating Eq.~\eqref{eq:harmonic_dqeff} for $N = 1$ at the initial conditions.
This is denoted by the narrow red shaded region surrounding the red line near
$I_{\rm 0} = 90^\circ$ in Fig.~\ref{fig:bin_comp}. We see that the scatter is
well predicted for these weakly-resonant conditions. We empirically choose the
transition between ``weakly'' and ``strongly'' resonant systems when the
$\abs{\Delta \bar{\theta}_{\rm e}}$ predicted by Eq.~\eqref{eq:harmonic_dqeff}
exceeds $6^\circ$.

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{LK90_plots/5_dWs_inner.png}
    \caption{Same as Fig.~\ref{fig:dWs} but for the LK-enhanced parameter
    regime. In the LK-enhanced regime, all $I_{\rm 0}$ merge within a Hubble
    time, and it is clear that $\overline{\Omega}_{\rm e} \approx \Omega_{\rm
    LK}$ can be satisfied for a wide range of inclinations. However, $e_{\max}
    \lesssim 0.8$ for these inclinations is moderate compared to the LK-induced
    regime.}\label{fig:dWs_inner}
\end{figure}
...
\begin{itemize}
    \item $\sin \p{\Delta I_{\rm eN}}$ is small unless $\mathcal{A} \simeq
        ...
        a LK cycle, and all the $\bm{\Omega}_{\rm eN}$ are aligned with
        $\overline{\bm{\Omega}}_{\rm e}$, implying all the $\Delta I_{\rm eN}
        \approx 0$.
...
\begin{figure}
    \centering
    \includegraphics[width=0.7\columnwidth]{LK90_plots/7_3vec_eta_cropped.png}
    \caption{Definition of angles in the case where $L / L_{\rm out}$ is
    nonzero. Similar to before, we choose the convention where $\bar{I}_{\rm
    tot,e} \in [0^\circ, 90^\circ]$ when $\overline{\Omega}_{\rm L} > 0$ and
    $\bar{I}_{\rm tot, e} \in [90^\circ, 180^\circ]$  when
    $\overline{\Omega}_{\rm L} < 0$. Here, $\bm{L}_{\rm out}$ is not fixed, but
    $\bm{L}_{\rm tot} \equiv \bm{L} + \bm{L}_{\rm out}$ is. Note that the
    coordinate system is now oriented with $\uv{Z} \propto \bm{L}_{\rm
    tot}$.}\label{fig:3vec_eta}
\end{figure}
...
negligible. When $\eta \neq 0$, $\bm{L}_{\rm out}$ is no longer fixed, but the
total angular momentum $\bm{L}_{\rm tot} \equiv \bm{L} + \bm{L}_{\rm out}$ is
...
octupole-order effects are negligible. To calculate the evolution of $\bm{L}$,
it is only necessary to evolve a single orbital element for the outer binary,
the inclination $I_{\rm tot, out}$ relative to the total angular momentum, in
addition to the orbital elements of the inner binary ($a$, $e$, $\ascnode$,
$I_{\rm tot}$, and $\omega$), as the ascending node of the outer binary
satisfies $\ascnode_{\rm out} = \ascnode + 180^\circ$. The equations of motion
are given by \citep{bin_diego}
\begin{align}
...
described in the corotating with $\bm{L}$ about $\bm{L}_{\rm tot}$ by the
equation of motion
\begin{equation}
    \p{\rd{\bm{S}}{t}}_{\rm rot} = \bm{\Omega}_{\rm e} \times \bm{S},
\end{equation}
where
\begin{align}
    \bm{\Omega}_{\rm e} &\equiv \Omega_{\rm SL}\uv{L}
        + \Omega_{\rm L} \uv{L}_{\rm tot},\nonumber\\
        &= \overline{\Omega_{\rm SL} \sin I_{\rm tot}}\uv{X}
            + \p{\overline{\Omega}_{\rm L}
            + \overline{\Omega_{\rm SL}\cos I_{\rm tot}}}\uv{Z},
            \label{eq:We_component_finiteeta}
\end{align}
where $\Omega_{\rm L} = -\rdil{\ascnode}{t}$ [Eq.~\eqref{eq:dWdt_eta}] is the
rate of precession of $\bm{L}$ about $\bm{L}_{\rm tot}$, and bars denote
averaging over an LK cycle. We define the averaged quantities
\begin{align}
    \overline{\Omega_{\rm SL} \sin I_{\rm out}} &\equiv
            \overline{\Omega}_{\rm SL} \sin \bar{I}_{\rm out},&
    \overline{\Omega_{\rm SL} \cos I_{\rm out}} &\equiv
            \overline{\Omega}_{\rm SL} \cos \bar{I}_{\rm out}.
\end{align}
...
\begin{equation}
    \cos \bar{\theta}_{\rm e} \equiv
        \frac{\overline{\bm{\Omega}}_{\rm e}}{\overline{\Omega}_{\rm e}}
            \cdot \overline{\bm{S}},
\end{equation}
where $\overline{\bm{S}}$ is the spin vector averaged over a LK cycle. The
orientation of $\overline{\bm{\Omega}}_{\rm e}$ is described by the inclination
...
At $t = t_f$, the inner binary is sufficiently compact that $\theta_{\rm sl}$ is
frozen (see bottom right panel of Fig.~\ref{fig:4sim_90_350}), and the system
satisfies $\mathcal{A} \gg 1$ ($\overline{\Omega}_{\rm SL} \propto a^{-5/2}$
while $\overline{\Omega}_{\rm L} \propto a^{3/2}$). When this is satisfied,
$\overline{\bm{\Omega}}_{\rm e} \parallel \bm{L}$, and so $\bar{\theta}_{\rm e,
f} = \theta_{\rm sl, f}$. Then, since adiabatic invariance implies
$\bar{\theta}_{\rm e, f} = \bar{\theta}_{\rm e, 0}$,
\begin{equation}
    \theta_{\rm sl, f} = \bar{\theta}_{\rm e, 0}.
\end{equation}

First, we consider the case where $\bm{S}_{\rm 0} = \uv{\overline{L}}_{\rm 0}$.
Then $\bar{\theta}_{\rm e, 0} = \abs{I_{\rm tot, 0} - \bar{I}_{\rm tot,e, 0}}$
(see Fig.~\ref{fig:3vec_eta}), and so
\begin{equation}
...
$\abs{\overline{\Omega}_{\rm L}} \gg \overline{\Omega}_{\rm SL}$, then
$\overline{\Omega}_{\rm e}$ is either parallel or anti-parallel to $\bm{L}_{\rm
tot}$ depending on whether $\overline{\Omega}_{\rm L}$ is positive or negative.
Define $I_{\rm c}$ to be the starting mutual inclination for which
$\overline{\Omega}_{\rm L}$ changes sign. Note that $I_{\rm c} > 90^\circ$:
even though $\Omega_{\rm L}$ changes sign at $90^\circ$, the inclination
decreases over a LK cycle for $I < I_{\lim}$ (where $I_{\lim} > 90^\circ$ is the
starting mutual inclination that maximizes $e_{\max}$ \citep{bin2}), so the sign
of $\Omega_{\rm L}$ changes over a LK cycle when $90^\circ < I_{\rm 0} <
I_{\lim}$. We find that
\begin{equation}
    \theta_{\rm sl, f} =
    \begin{cases}
        I_{\rm tot, 0} & I_{\rm 0} < I_{\rm c},\\
        180^\circ - I_{\rm tot, 0} & I_{\rm 0} > I_{\rm c}.
    \end{cases}
\end{equation}
...
between $\bm{S}$ and $\bm{L}_{\rm tot}$, giving
\begin{equation}
    \theta_{\rm sl, f} =
    \begin{cases}
...
binary parameters as the fiducial system before, but use a tertiary companion
with $m_3 = 30M_{\odot}$, $a_{\rm out} = 4500\;\mathrm{AU}$, and $e_{\rm out} =
0$. Fig.~\ref{fig:finite_qslfscan} shows that Eq.~\eqref{eq:qslf_eta_pred}
accurately predicts $\theta_{\rm sl, f}$ when $\theta_{\rm sl, 0} = 0^\circ$ for
this parameter regime when conservation of $\bar{\theta}_{\rm e}$ is good.
Furthermore, deviations from exact $\bar{\theta}_{\rm e}$ conservation are well
described by Eq.~\eqref{eq:qslf_plot_black}, the prediction of the theory in
Section~\ref{s:fast_merger}. Unlike the $\eta = 0$ case
(Fig.~\ref{fig:qslscan}), $\theta_{\rm sl, f}$ is not symmetric about $I_{\rm c}
\approx 92.14^\circ$. This is because $\abs{\Omega}_{\rm L}$ is not exactly
equal on either side of $I_{\rm c}$. Additionally, unlike in the $\eta = 0$
case, the minimum $\theta_{\rm sl,f}$ is not exactly zero. We showed in
Section~\ref{ss:effect} that when $\eta = 0$, $\bm{L}$ is fixed when $I_{\rm 0}
= 90^\circ$, as $\rdil{\ascnode}{t} = \rdil{I}{t} = 0$. For nonzero $\eta$,
$\rdil{\ascnode}{t}$ and $\rdil{I_{\rm tot}}{t}$ are not zero at $I_{\rm 0} =
I_{\rm c}$.

Eq.~\eqref{eq:qslf_eta_pred} also gives good agreement LK-enhanced menger
regime. We consider the same inner binary parameters as in
Section~\ref{ss:lk_enhanced} but use a tertiary companion with $m_3 =
30M_{\odot}$, $a_{\rm out} = 3\;\mathrm{AU}$, and $e_{\rm out} = 0$. The results
are shown in Fig.~\ref{fig:finite_bin_comp}.
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{LK90_plots/8finite_qslfs.png}
    ...
    black line denotes $I_{\rm c} \approx 92.14^\circ$, the inclination for
    which $\overline{\Omega}_{\rm L}$ changes signs, and the two horizontal
    lines denote the predictions of Eq.~\eqref{eq:qslf_eta_pred}. The dotted
    black lines bound the deviation due to non-adiabatic evolution, given by
    Eq.~\eqref{eq:qslf_plot_black}.
    }\label{fig:finite_qslfscan}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{LK90_plots/8bin_comp.png}
    \caption{Similar to Fig.~\ref{fig:bin_comp} except with stellar mass
    tertiary $m_3 = 30M_{\odot}$ and $\tilde{a}_{\rm out} =
    3\;\mathrm{AU}$.}\label{fig:finite_bin_comp}
\end{figure}

\section{Conclusion and Discussion}\label{s:discussion}

In this paper, we consider the evolution of the final spin-orbit misalignment
angle $\theta_{\rm sl}$ of LK-induced binary BH mergers. We study cases where
the tertiary companion is either a supermassive black hole (SMBH) or a stellar
mass black hole (BH). We show that, when the tertiary orbit is circular and
octupole effects are unimportant, the spin vectors of the inner BHs obey the
simple equation of motion Eq.~\eqref{eq:dsdt_weff}. Analysis of this equation
...
\begin{itemize}
    \item For most parameters of astrophysical relevance, the angle
        ...
        perfect adiabaticity can be predicted from initial conditions [see
        Eq.~\ref{eq:prediction} and Fig.~\ref{fig:deviations}].

    \item When the resonant condition $\bar{\Omega}_{\rm e} \approx N\Omega_{\rm
        LK}$ for integer $N$ is satisfied, significant oscillations in
        $\bar{\theta}_{\rm e}$ can arise. We derive an analytic estimate of this
        oscillation amplitude [Eq.~\eqref{eq:harmonic_dqeff}]. This estimate
        both demonstrates that the resonance is unimportant for ``LK-induced''
        mergers and qualitatively explains the scatter in $\theta_{\rm sl, f}$
        seen by LL17 (see Fig.~\ref{fig:bin_comp}).
\end{itemize}

In previous work, LL18 found that the final spin-orbit misalignment angle
$\theta_{\rm sl, f}$ exhibits a $90^\circ$ attractor (Fig.~\ref{fig:qslscan})
when the octupole effect is negligible. Our theory shows that this attractor is
a result of their taking $\theta_{\rm sl, 0} = 0^\circ$ \citep[in agreement
with][]{yu2020spin}, as the initial and final spin orientations are related via
conservation of $\bar{\theta}_{\rm e}$ (Sections~\ref{ss:effect}
and~\ref{s:stellar}). Note that the component BH spins in the recent LIGO
detection GW190521 have spin-orbit misalignment angle $\sim 90^\circ$
\citep{190521}. If the evolution history of the component BHs resembled our
LK-induced scenario, a primordial $\theta_{\rm sl, 0} \approx 0^\circ$ would be
consistent with the observed outcome.

The systems detected by LIGO in O1 and O2 have $\chi_{\rm eff} \approx 0$
, where $\chi_{\rm eff}$ is given by Eq.~\eqref{eq:chi_eff}
\citep[e.g.][]{Abbott:2016blz, abbott2019binary}. One possible explanation for
this is that BHs are born slowly rotating \citep[e.g.][]{fuller2019most}. Our
mechanism provides an alternative origin for this behavior with no assumptions
on the BH spin magnitudes if $\theta_{\rm sl, 0} \approx 0^\circ$. We also
quantify the expected scatter from the perfect $90^\circ$ attractor reported by
LL18 due to either non-adiabaticity or resonances during the inspiral.

When the tertiary orbit is eccentric, the octupole-order contributions can no
longer be neglected. The octupole-order LK effect is not integrable, and the
eccentricity excitations are no longer regular \citep[see e.g.][]{bin2}. In this
case, $\bm{\Omega}_{\rm e}$ has neither consistent direction nor magnitude, and
our theory cannot be applied. In fact, the resulting $\theta_{\rm sl, f}$
distribution is largely unrelated to the initial $\bar{\theta}_{\rm e, 0}$
\citep{bin2}.

If the system is not sufficiently hierarchical ($a_{\rm out}$ is too small), the
double averaging approximation breaks down, and the dynamics are no longer
integrable, so there is little reason to expect any relation between
$\theta_{\rm sl, f}$ and $\bar{\theta}_{\rm e, 0}$. However, \citet{bin2} found
that the double averaged equations predict the correct merger window and merger
fractions even beyond their regime of validity if the octupole effect is weak,
so it is possible that our results concerning $\theta_{\rm sl, f}$ are also
somewhat robust even when the double averaged equations formally break down. We
defer this possibility to future work.

Finally, in this work, we only follow the evolution of $\theta_{\rm sl}$ until
the orbital separation is $\leq 0.5\;\mathrm{AU}$ for the LK-induced mergers,
and $\leq 0.1\;\mathrm{AU}$ for the LK-enhanced mergers. To leading
post-Newtonian (PN) order, $\theta_{\rm sl}$ is constant for separations smaller
than these until the spin-spin interaction (2 PN) becomes important
[Eq.~\eqref{eq:dsdt}]. This interaction is negligible until the binary enters
the LIGO band \citep{bin3, yu2020spin}.
...
behavior of the system. Additionally, Eq.~\eqref{eq:app_single_resonance} finds
a natural interpretation in this formulation, and its accuracy is numerically
...
$\tilde{\bm{A}}\bm{S} = \bm{\Omega}_{\rm e} \times \bm{S}$, then the equation of
motion is
\begin{align}
    \p{\rd{\bm{S}}{t}}_{\rm rot}
        &= \tilde{\bm{A}} \bm{S}\label{eq:app_dsdt_weff}.
\end{align}
Here, $\tilde{\bm{A}}$ is periodic with period $T_{\rm LK}$.

\subsection{Without Nutation}\label{app:a1}

First, for simplicity, let us assume that $\bm{\Omega}_{\rm e}$ does not nutate,
so its orientation is fixed. In this case, Eq.~\eqref{eq:app_dsdt_weff} admits
an exact conserved quantity:
\begin{equation}
    \rd{}{t}\s{e^{-\Phi}\bm{S}} = 0,\label{eq:app_cons}
\end{equation}
where
\begin{equation}
    \Phi(t) \equiv \int\limits^t \tilde{\bm{A}}\;\mathrm{d}t.
\end{equation}
Separately, since Eq.~\eqref{eq:app_dsdt_weff} is linear and has periodic
coefficients, Floquet theory tells us that $\bm{S}\p{t + T_{\rm LK}}$ is related
to $\bm{S}(t)$ by the monodromy matrix $\tilde{\bm{M}}$:
\begin{equation}
    \bm{S}\p{t + T_{\rm LK}} = \tilde{\bm{M}}\bm{S}(t).\label{eq:app_M}
\end{equation}
Comparing Eqs.~\eqref{eq:app_cons} and~\eqref{eq:app_M}, we immediately find
that
\begin{align}
    \tilde{\bm{M}} &= \exp\s{
            \int\limits_0^{T_{\rm LK}}\tilde{\bm{A}}\;\mathrm{d}t}
            ,\nonumber\\
        &= \exp\s{T_{\rm LK} \overline{\tilde{\bm{A}}}},\label{eq:app_Mbase}
\end{align}
where again the overline denotes time averaging. As $\tilde{\bm{M}}$ is a
rotation matrix, call $\bm{R}$ its rotation axis, or its eigenvector with
eigenvalue $1$. But $\bm{R} = \hat{\bm{\Omega}}_{\rm e}$, so $\bm{S} \cdot
\hat{\bm{\Omega}}_{\rm e}$ must be constant for every $t = NT_{\rm LK}$.

This example is somewhat trivial, as if $\bm{\Omega}_{\rm e}$ does not nutate,
then $\bm{S}$ just precesses around fixed $\bm{R} = \hat{\bm{\Omega}}_{\rm e}$
at a variable rate, and $\bm{S} \cdot \hat{\bm{\Omega}}_{\rm e}$ is conserved.
...
Fig.~\ref{fig:4sim_90_350_zoom} shows that $\bm{\Omega}_{\rm e}$ nutates
substantially within a LK period when $\mathcal{A} \simeq 1$ (nutation is
equivalent to $\Omega_{\rm e1} \neq 0$ and $\Delta I_{\rm e1} \neq 0^\circ$). We
infer that, even when $\bm{\Omega}_{\rm e}$ does nutate appreciably, there are
...
When $\bm{\Omega}_{\rm e}$ is allowed to nutate within $T_{\rm LK}$, the
quantity given by Eq.~\eqref{eq:app_cons} is no longer conserved, as
\begin{align}
    \rd{}{t}\s{e^{-\Phi}\bm{S}} = e^{-\Phi}\rd{\bm{S}}{t}
        - \tilde{A}e^{-\Phi}\bm{S} \neq e^{-\Phi}\s{\rd{\bm{S}}{t} -
        \tilde{\bm{A}}\bm{S}} = 0.
\end{align}
Instead, we define new quantities $\Phi'$ and $\tilde{\bm{B}}$ defined such that
\begin{align}
    \Phi'(t) &\equiv \int\limits^t
        \tilde{\bm{A}} + \tilde{\bm{B}}\;\mathrm{d}t,\\
    \rd{}{t}\s{e^{-\Phi'}\bm{S}} &= 0.
\end{align}
This requires
\begin{equation}
    \tilde{\bm{B}} = \s{e^{-\Phi'}, \tilde{\bm{A}}}
        e^{\Phi'},\label{eq:app_bexact}
\end{equation}
where the square brackets denote the commutator. The monodromy matrix is then
\begin{equation}
    \tilde{\bm{M}} = \exp\s{
        \int\limits_0^{T_{\rm LK}}\tilde{\bm{A}} + \tilde{\bm{B}}
...
\begin{equation}
    \tilde{\bm{M}} = e^{T_{\rm LK}\overline{\tilde{\bm{A}}}}
        e^{T_{\rm LK}\overline{\tilde{\bm{B}}}}
        \exp\s{-\frac{T_{\rm LK}^2\s{\overline{\tilde{\bm{A}}},
            \overline{\tilde{\bm{B}}}}}{2} +\dots}.\label{eq:m_gen}
\end{equation}
Note that $\tilde{\bm{M}}$ is a rotation matrix (see
Section~\ref{ss:monodromy}), and $\exp\s{T_{\rm LK}\overline{\tilde{\bm{A}}}}$
is also a rotation matrix ($\tilde{\bm{A}}$ is skew-symmetric), so the remainder
of the right hand side above must also be a rotation matrix (as the rotation
matricies are closed under matrix multiplication). For convenience, define
\begin{align}
    \tilde{\bm{R}}_{\rm A} &\equiv
        e^{T_{\rm LK}\overline{\tilde{\bm{A}}}}&
    \tilde{\bm{R}}_{\rm B} &\equiv
        e^{T_{\rm LK}\overline{\tilde{\bm{B}}}}
        \exp\s{-\frac{T_{\rm LK}^2\s{\overline{\tilde{\bm{A}}},
            \overline{\tilde{\bm{B}}}}}{2} +\dots},
\end{align}
where $\tilde{\bm{R}}_{\rm A}$ and $\tilde{\bm{R}}_{\rm B}$ are rotation
matricies that effect rotations by angles $\theta_{\rm A}$ and $\theta_{\rm B}$
about their respective axes.

When can $\tilde{\bm{R}}_{\rm B}$ be neglected? From Eq.~\eqref{eq:app_bexact}, we see
that $\tilde{\bm{B}} = 0$ vanishes ($\theta_{\rm B} = 0$) when $\s{\tilde{\bm{A}},
\Phi}$ commute, which occurs when $\bm{\Omega}_{\rm e}$ does not nutate, and we
...
small $\theta_{\rm B}$ alone is not sufficient to assume $\tilde{\bm{M}} \approx
\tilde{\bm{R}}_{\rm A}$. To see this, note if $\theta_{\rm B}$ is small, then
$\tilde{\bm{R}}_{\rm B} \approx \bm{1}$ where $\bm{1}$ is the $3\times3$
...
$\tilde{\bm{M}} \approx \tilde{\bm{R}}_{\rm A}$ and $\tilde{\bm{M}} \approx
\tilde{\bm{R}}_{\rm A}$ as before. However, if $\overline{\Omega}_{\rm e}T_{\rm
LK} \approx 2\pi N$ for integer $N$, then $\tilde{\bm{R}}_{\rm A}$ itself is
near the identity as well, and $\tilde{\bm{R}}_{\rm B}$ \emph{cannot} be
neglected when calculating $\tilde{\bm{M}}$. The criterion for neglecting
$\tilde{\bm{R}}_{\rm B}$ is then clear: $\theta_{\rm B}$ must be much closer to
$2\pi N$ for integer $N$ than $\theta_{\rm A}$.

To complete this picture, we argue that, for the spin dynamics studied in the
main text, $\theta_{\rm B}$ is small, so generally $\tilde{\bm{R}}_{\rm B}
\approx \bm{1}$, and $\overline{\Omega}_{\rm e} \approx N\Omega_{\rm LK}$ is a
\emph{necessary} condition for $\bm{R}$ to differ significantly from
$\hat{\overline{\bm{\Omega}}}_{\rm e}$. To do this, we recall that
...
\begin{equation}
    \tilde{\bm{B}} \approx
        \s{e^{-\Phi},\tilde{\bm{A}}}e^{\Phi}.\label{eq:bapprox}
\end{equation}
Next, recall that $\Phi$ is the integral of $-\tilde{\bm{A}}$, and so the
magnitude of $\overline{\tilde{\bm{B}}}$ [$\tilde{\bm{B}}$ primarily affects
$\tilde{\bm{M}}$ via its average, see Eq.~\eqref{eq:app_Mgen}] depends on the
average misalignment between the vectors $\bm{\Omega}_{\rm e}$ and $\int^t
\bm{\Omega}_{\rm e}\;\mathrm{d}t \simeq \overline{\bm{\Omega}}_{\rm e}$. There
are a few possible regimes to consider: (i) if $\mathcal{A} \gg 1$, then LK
oscillations are frozen, and $\bm{\Omega}_{\rm e}$ does not nutate; (ii) if
$\mathcal{A} \ll 1$, then $\uv{\Omega}_{\rm e} \approx \uv{L}_{\rm tot} \approx
\hat{\overline{\bm{\Omega}}}_{\rm e}$ for almost all of $T_{\rm Lk}$; and (iii)
if $\mathcal{A} \simeq 1$, then $e_{\max}$ cannot be too large
[Eq.~\eqref{eq:jemaxepsgr}], and so $\Omega_{\rm L}$ and $\Omega_{\rm SL}$
cannot vary too much within an LK cycle and the nutation of $\bm{\Omega}_{\rm
e}$ is limited. This analysis suggests that, at least far from resonance, the
nutation of $\bm{\Omega}_{\rm e}$ is limited, and the commutator in
...
$\overline{\Omega}_{\rm e} = N\Omega_{\rm LK}$, as claimed in the main text, and
that otherwise $\bm{R} \parallel \overline{\bm{\Omega}}_{\rm e}$.

% useful but also not the best way of arguing this
% \begin{equation}
%     \int\limits_0^{T_{\rm LK}}
%         \s{e^{T_{\rm LK}\overline{\tilde{\bm{A}}}}, \tilde{\bm{A}}}
%             e^{-T_{\rm LK}\overline{\tilde{\bm{A}}}}\;\mathrm{d}t
%         = 0.
% \end{equation}
...
% \begin{equation}
%     \s{e^{-s\Phi'}, \tilde{\bm{A}}}e^{s\Phi'} = \rd{}{s}
%         \s{e^{-s\Phi'}\s{\tilde{\bm{A}}, \Phi'}e^{s\Phi'}},
% \end{equation}
% and so
% \begin{equation}
%     \tilde{\bm{B}} = \int\limits_0^1
%         e^{-s\Phi'}\s{\tilde{\bm{A}}, \Phi'}e^{s\Phi'}\;\mathrm{d}s.
% \end{equation}
% This equation is hard to solve exactly. However, we note that a leading-order,
% self-consistent solution can be found when the nutation of $\bm{\Omega}_{\rm e}$
% is small. We proceed under two assumptions: first, that $\tilde{\bm{B}} \ll
% \tilde{\bm{A}}$, and second, that $\overline{\Omega}_{\rm e} \leq \Omega_{\rm
...
% matricies, so if the angle of rotation of $e^{-\Phi'}$ is too close to $2\pi N$
% for integer $N$, $\tilde{B} \ll \tilde{A}$ does not imply that $e^{-\Phi'}
% \approx e^{-\Phi}$.

% Let's assume that $\tilde{\bm{B}} \ll \tilde{\bm{A}}$ and furthermore
% that $e^{-\Phi'} \approx e^{-\Phi}$ for $t \in [0, T_{\rm LK}]$ (this condition
% is further described below). Then $\tilde{\bm{B}}$
% can be approximated by
% \begin{equation}
%     \tilde{\bm{B}} \approx \int\limits_0^1
%         e^{-s\Phi}\s{\tilde{\bm{A}}, \Phi}e^{s\Phi}\;\mathrm{d}s.
% \end{equation}
% When the nutation of $\bm{\Omega}_{\rm e}$ is small, the rotation axis of $\Phi$
% is generally nearly aligned with that of $\tilde{\bm{A}}$, so the commutator is
% small (becoming zero when nutation is completely ignored, as in the previous
% section), and indeed $\tilde{\bm{B}} \ll \tilde{\bm{A}}$ as we assumed. Thus,
% when nutation is small, $\tilde{B} \ll \tilde{A}$.

% However, $\tilde{\bm{B}} \ll \tilde{\bm{A}}$ is not a sufficient condition to
...
% $\exp\p{-T_{\rm LK}\overline{\tilde{\bm{A}}}} \approx \bm{1}$ the identity
% matrix.

% a x (b x c) = b(a.c) - c(a.b)     |      b x (a x c) = a(b.c) - c(b.a)
% commutator = b(a.c) - a(b.c)
% We denote for convenience the vector $\bm{F}$ satisfying
% \begin{equation}
%     \Phi \bm{S} \equiv \bm{F} \times \bm{S}
%         = \s{-\int\limits^t \bm{\Omega}_{\rm e}(t)\;\mathrm{d}t} \times \bm{S}.
% \end{equation}
% Note that
% then we find that
% \begin{equation}
%     \s{\Phi, \tilde{\bm{A}}}\bm{S} = \bm{\Omega}_{\rm e}\p{\bm{F} \cdot \bm{S}}
%         - \bm{F}\p{\bm{\Omega}_{\rm e} \cdot \bm{S}}.
% \end{equation}
...
monodromy matrix $\tilde{\bm{M}}$. In this section, we aim to reconcile this
...
$\overline{\Omega}_{\rm e} \approx N\Omega_{\rm LK}$ with amplitude given by
Eq.~\eqref{eq:harmonic_dqeff}. On the other hand, in Appendix~\ref{app:a2}, we
found that $\bm{R}$ is aligned with $\overline{\bm{\Omega}}_{\rm e}$ except when
$\overline{\Omega}_{\rm e}$ is sufficiently close to $N\Omega_{\rm LK}$ that
nutation of $\bm{\Omega}_{\rm e}$ becomes important, but we were not able to
determine a closed-form criterion for the misalignment. In this section, we show
numerically that the formulas given in the main text give good predictions for
the orientation of $\bm{R}$.
\begin{figure}
    \centering
    \includegraphics[width=0.5\columnwidth]{LK90_plots/7_3vec_mc.png}
    \caption{Definition of angles for numerical study of the monodromy matrix
    rotation axis. $\bm{R}$ is the eigenvector of the monodromy matrix
    $\tilde{\bm{M}}$ with eigenvalue $1$.}\label{fig:app_vecs}
\end{figure}

To validate the analytic prediction given by Eq.~\eqref{eq:harmonic_dqeff}, we
numerically compute $\tilde{\bm{M}}$. We study the $\eta \neq 0$, LK-enhanced
regime ($m_1 = m_2 = m_3 = 30M_{\odot}$, $a_{\rm 0} = 0.1\;\mathrm{AU}$, $e_{\rm
...
initial inclinations of the inner binary, we construct $\tilde{\bm{M}}$ by
evolving the spin equation of motion Eq.~\eqref{eq:dsdt_weff} starting with the
three initial conditions $\bm{S} = \uv{x}$, $\bm{S} = \uv{y}$, and $\bm{S} =
\bm{z}$ (see Fig.~\ref{fig:app_vecs})
over a single LK period, then using
\begin{equation}
    \tilde{\bm{M}} = \bm{\Phi}\p{T_{\rm LK}} \bm{\Phi}^{-1}(0)
        = \bm{\Phi}\p{T_{\rm LK}},
\end{equation}
where $\bm{\Phi}(t)$ is the \emph{principal fundamental matrix solution} whose
columns are solutions to Eq.~\eqref{eq:dsdt_weff} and $\bm{\Phi}(0)$ is the
identity. $\bm{R}$ is then the eigenvector that has eigenvalue $1$. Note that if
$\bm{v}$ is an eigenvector, so too is $-\bm{v}$; we choose convention that
$\bm{R}$ points in the same direction as $\overline{\bm{\Omega}}_{\rm e}$, i.e.\
$\Delta I_{\rm m} \equiv \abs{I_{\rm m} - \bar{I}_{\rm e}} < 90^\circ$.

The orientation of $\bm{R}$ is related to the $\abs{\Delta \bar{\theta}_{\rm
e}}$ predicted by Eq.~\eqref{eq:harmonic_dqeff}: if $\bm{R}$ and
$\overline{\bm{\Omega}}_{\rm e}$ are misaligned by angle $\Delta I_{\rm m}$,
then $\bar{\theta}_{\rm e}$ oscillates with semi-amplitude $\Delta I_{\rm m}$.
Thus, we can infer $\Delta I_{\rm m}$ in the vicinity of each
$\overline{\Omega}_{\rm e} = N\Omega_{\rm LK}$ resonance from
Eq.~\eqref{eq:harmonic_dqeff}. If the total $\Delta I_{\rm m}$ is well
approximated by the sum of the contribution from each $N$ harmonic, then we have
\begin{equation}
    \Delta I_{\rm m} \sim
        \sum\limits_{N = 1}^{\infty} \frac{h(N)}{2}
        \abs{\frac{\sin \Delta I_{\rm eN}
        \Omega_{\rm eN}}{\overline{\Omega}_{\rm e} - N\Omega_{\rm LK}}}.
        \label{eq:app_im_pred}
\end{equation}
Fig.~\ref{fig:finite_bin_comp_m} shows that this calculation predicts the
numeric $\Delta I_{\rm m}$ well. This supports our assertion that $\bm{R}$
deviates from $\hat{\overline{\bm{\Omega}}}_{\rm e}$ at resonances
$\overline{\Omega}_{\rm e} \approx N \Omega_{\rm LK}$, and the misalignment is
captured by Eq.~\eqref{eq:harmonic_dqeff}.

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{LK90_plots/8bin_comp_m.png}
    \caption{Comparison of the orientation $\bm{R}$ obtained from numerical
    simulations of Eq.~\eqref{eq:dsdt_weff} in the $\eta \neq 0$, LK-enhanced
    parameter regime ($m_1 = m_2 = m_3 = 30M_{\odot}$, $a_{\rm 0} =
    0.1\;\mathrm{AU}$, $e_{\rm 0} = 10^{-3}$,$\tilde{a}_{\rm out} =
    ...
    $\bm{R}$ and $\overline{\bm{\Omega}}_{\rm e}$ as the black line. Separately,
    ...
    within the $N = 1$ resonance, which is not predicted by our simple
