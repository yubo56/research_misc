...
\title[BH Triple Spin-Orbit Dynamics]{Spin Dynamics in Hierarchical Black Hole
...
As the \textcolor{Corr}{Laser Interferometer Gravitational-Wave Observatory
(LIGO) Scientific Collaboration} continues to detect mergers of black hole (BH)
...
binary angular momentum axis ($\theta_{\rm sl, i} = 0$), it evolves towards
a perpendicular state ($\theta_{\rm sl, f} = 90^\circ$) near merger.
...
$\theta_{\rm sl, f}$ varies when the initial inclination of the tertiary
orbit $I_{\rm i}$ (relative to the inner orbit) is varied. Note that for rapid mergers
(when $I_{\rm i}$ is close to $90^\circ$), the attractor does not exist; as
$I_{\rm i}$
deviates more from $90^\circ$, the merger time increases and $\theta_{\rm
sl, f}$ is close to $90^\circ$.  This $90^\circ$ attractor gives rise to a
...
Section~\ref{s:lk_enhanced}, we comment on the LK-enhanced scenario.
\textcolor{Corr}{In Section~\ref{s:stellar}, we generalize our results to
stellar-mass tertiary companions.} We discuss and conclude in
...
\begin{figure}
    ...
    misalignment angle $\theta_{\rm sl}$. The unit of time $t_{\rm LK, i}$
    is the LK timescale [Eq.~\eqref{eq:t_lk}] evaluated for the initial
    conditions. The inner binary has $m_1 = 30M_{\odot}$, $m_2 = 20M_{\odot}$,
    and initial $a_{\rm i} = 100\;\mathrm{AU}$, $e_{\rm i} = 0.001$, $I_{\rm i}
    = 90.35^\circ$ (with respect to the outer binary), and $\theta_{\rm sl, i} =
    0$. The tertiary SMBH has $a_{\rm out} = 2.2\;\mathrm{pc}$, $e_{\rm out} =
    0$, and $m_3 = 3 \times 10^7 M_{\odot}$. It can be seen that $\theta_{\rm
    sl}$ evolves to $\sim 90^\circ$ as $a$ shrinks to smaller
    values.}\label{fig:4sim_90_350}
\end{figure}
\begin{figure}
    \centering
    ...
    $\theta_{\rm sl, f}$ as a function of the initial inclination $I_{\rm i}$
    for LK-induced mergers. The other parameters are the same as those in
    Fig.~\ref{fig:4sim_90_350}. For $I_{\rm i}$ somewhat far away from
    $90^\circ$, the resulting $\theta_{\rm sl, f}$ are all quite near
    $90^\circ$. \textcolor{Corr}{The horizontal black solid line shows the
...
initial $a = 100\;\mathrm{AU}$, $e_{\rm i} = 0.001$, and $I_{\rm i} = 90.35^\circ$. We take
...
$e_{\max}$ as a function of the initial $I_{\rm i}$ (and initial $e_{\rm i} \ll 1$).
The largest value of $e_{\max}$ occurs at $I_{\rm i} = 90^\circ$ and is given
by
\begin{equation}
    j(e_{\max})_{I_{\rm i} = 90^\circ} = (8/9) \epsilon_{\rm GR}.
        \label{eq:jemaxepsgr}
\end{equation}
...
$\epsilon_{\rm GR} \ll 1$ at $a = a_{\rm i}$, leading to $e_{\max} \sim 1$ within a
narrow inclination window around $I_{\rm i} = 90^\circ$.
...
\end{equation}
We write $\overline{\bm{\Omega}}_{\rm e} \equiv \bm{\Omega}_{\rm e0}$ for
...
0.1\;\mathrm{AU}$, $e_{\rm i} = 10^{-3}$, $I_{\rm i} = 70^\circ$, $a_{\rm out} =
...
\begin{figure}
    ...
    \times 10^7 M_{\odot}$, $a = 0.1\;\mathrm{AU}$, $e_{\rm i} = 10^{-3}$,
    $I_{\rm i} = 70^\circ$, $a_{\rm out} = 300\;\mathrm{AU}$, $e_{\rm out} = 0$,
    and initial condition $\theta_{\rm sl, i} = 0$. For each mass ratio $m_1 /
...
$\overline{\bm{\Omega}}_{\rm e} = \bm{\Omega}_{\rm e0}$ [see
...
have $\bm{\Omega}_{\rm e} \simeq \overline{\bm{\Omega}}_{\rm e}$ (and
...
\begin{figure}
    ...
    that for $I_{\rm i} > 90^\circ$ (and $\bar{I} > 90^\circ$), we choose
...
binary merger starting with $I_{\rm i} = 90.2^\circ$, for which $\abs{\Delta
...
\begin{figure*}
    ...
    \caption{Same as Fig.~\ref{fig:4sim_90_350_zoom} except for $I_{\rm i} =
...
\begin{equation}
    j_{\star} \equiv j(e_{\star}) = f
        \sqrt{\frac{5}{3}\cos^2 I_{\rm i}},\label{eq:jstar_ansatz}
\end{equation}
where the star subscript denotes evaluation at $\mathcal{A} \simeq 1$ and $f>1$
is a constant. Eq.~\eqref{eq:jstar_ansatz} assumes that $I_{\rm i}$ far enough from
$90^\circ$ that the GR effect is unimportant in determining $e_{\max}$. The
value of f turns out to be relatively insensitive to $I_{\rm i}$.
...
holds across a range of $I_{\rm i}$), we fit the last remaining free parameter $f$
[Eq.~\eqref{eq:jstar_ansatz}] to the data from numerical simulations. This
yields $f \approx 2.72$, leading to
\begin{align}
    \abs{\frac{\dot{\bar{I}}_{\rm e}}{\overline{\Omega}_{\rm e}}}_{\max}
        \simeq{}& 0.98^\circ \p{\frac{\cos I_{\rm i}}{\cos (90.3^\circ)}}^{-37/8}
...
\begin{figure}
    ...
    start of inspiral $t_{\rm LK, i}$. The agreement between the analytical
    and numerical results is excellent for $T_{\rm m} \gg t_{\rm
    LK, i}$.}\label{fig:good_quants}
\end{figure}
...
carry out simulations for a range of $I_{\rm i}$, and for each $I_{\rm i}$ we consider $100$
...
\begin{figure}
    ...
    function of initial inclination $I_{\rm i}$. For each $I_{\rm i}$, $100$ simulations are
...
i}$ and $t = t_{\rm f}$, i.e.
\begin{equation}
    \bar{\theta}_{\rm e, f} \simeq \bar{\theta}_{\rm e, i}.
...
\begin{equation}
    \theta_{\rm sl, f} \simeq \bar{\theta}_{\rm e, i},
...
Now, first consider the case where the initial spin $\bm{S}_{\rm i}$ is aligned
with the initial $\bm{L}_{\rm i}$. This initial spin is inclined with respect to
$\overline{\bm{\Omega}}_{\rm e}$ by $\bar{\theta}_{\rm e, i} = \abs{I_{\rm i} -
\bar{I}_{\rm e, i}}$, where $I_{\rm i}$ is the initial inclination angle
between $\bm{L}$ and $\bm{L}_{\rm out}$ and $\bar{I}_{\rm e, i}$ is the
initial value of $\bar{I}_{\rm e}$. Thus, adiabatic invariance implies
\begin{equation}
    \theta_{\rm sl, f} \simeq \abs{I_{\rm i} - \bar{I}_{\rm e, i}}.
\end{equation}
In the special case where the binary initially satisfies $\overline{\Omega}_{\rm
SL} \ll \abs{\overline{\Omega}_{\rm L}}$ or $\abs{\mathcal{A}_{\rm i}} \ll 1$,
we find that $\overline{\bm{\Omega}}_{\rm e}$ is nearly parallel to $\bm{L}_{\rm
out}$ (for $I_{\rm i} < 90^\circ$) or antiparallel to $\bm{L}_{\rm out}$ (for
$I_{\rm i} > 90^\circ$). Thus,
\begin{equation}
    \theta_{\rm sl, f} =
    \begin{cases}
        I_{\rm i} & I_{\rm i} < 90^\circ,\\
        180^\circ - I_{\rm i} & I_{\rm i} > 90^\circ.
    \end{cases}
\end{equation}
Since LK-induced mergers necessarily require $I_{\rm i}$ close to $90^\circ$, we
find that $\theta_{\rm sl, f}$ is ``attracted'' to $90^\circ$.

Eq.~\eqref{eq:4_4_general} can be applied to more general initial spin
orientations. For initial $\abs{\mathcal{A}_{\rm i}} \ll 1$ (as required for
LK-induced mergers), $\overline{\Omega}_{\rm e, i}$ is parallel to $\pm
\bm{L}_{\rm out}$. Suppose the initial inclination between $\bm{S}$ and
$\bm{L}_{\rm out}$ is $\theta_{\rm s,out, i}$, then $\bar{\theta}_{\rm
e, i} = \theta_{\rm s,out, i}$ or $180^\circ - \theta_{\rm s,
out, i}$ (depending on whether $I_{\rm i} < 90^\circ$ or $I_{\rm i} >
90^\circ$). Thus,
\begin{equation}
    \theta_{\rm sl, f} \simeq
    \begin{cases}
        \theta_{\rm s,out, i} & I_{\rm i} < 90^\circ,\\
        180^\circ - \theta_{\rm s,out, i} & I_{\rm i} > 90^\circ,
    \end{cases}
\end{equation}
...
\begin{equation}
    \abs{\theta_{\rm sl, f} - \bar{\theta}_{\rm e, i}}
...
$\theta_{\rm sl, f}$ from $\sim 90^\circ$ except very near $I_{\rm i} =
90^\circ$. This is expected, as Eq.~\eqref{eq:prediction} is not very accurate
very near $I_{\rm i} = 90^\circ$, where it diverges (see Fig.~\ref{fig:deviations}).

When $I_{\rm i} = 90^\circ$ exactly, Fig.~\ref{fig:qslscan} shows that $\theta_{\rm
sl, f} = 0^\circ$. This can be understood: $I_{\rm i} = 90^\circ$ gives
...
$\theta_{\rm sl, i} = 0$, so $\theta_{\rm sl, f} = 0$.
...
When $\theta_{\rm sl, i} = 0$, we have $\Phi(t_{\rm i}) = 0$, as $\bm{S}$
starts in the $\uv{x}$-$\uv{z}$ plane. Then, as $I_{\rm i}$ is smoothly varied, the
...
\bar{\theta}_{\rm e}}_{\rm f}$ as $I_{\rm i}$ is changed, giving rise to the
...
\textcolor{Corr}{In the previous section, we showed that the final $\theta_{\rm
sl, f}$ distribution can be well understood when assuming the $N \geq 1$ Fourier
harmonics in Eq.~\eqref{eq:dsdt_fullft} can be neglected. In this section, we
study the effects of these neglected terms and argue that they are not important
in the LK-induced merger regime.} A regime for which they cannot be neglected is
discussed separately in Section~\ref{s:lk_enhanced}.
...
\end{align}
Resonances may occur when $\overline{\Omega}_{\rm e} = N\Omega_{\rm LK}$.
\textcolor{Corr}{
Assuming that any resonances that do exist are well separated, we restrict our
attention to a single $N$ at a time. There are then two time-dependent
perturbations, a modulation of the oscillation frequency and a driving term. We
analyze these two terms separately to identify resonant behaviors.}
...
\begin{equation}
    \rd{S_\perp}{t} \approx \s{i\overline{\Omega}_{\rm e}
        + \cos\p{\Delta I_{\rm eN}} \Omega_{\rm eN}\cos\p{N\Omega_{\rm LK}t}}
            S_\perp.
\end{equation}
This has the exact solution
\begin{align}
    S_\perp(t) &= S_\perp(t_{\rm i})
        \exp\s{i\overline{\Omega}_{\rm e}t + \frac{\cos\p{\Delta I_{\rm eN}}
            \Omega_{\rm eN}}{N\Omega_{\rm LK}}\sin\p{N\Omega_{\rm LK}\tau}}.
\end{align}
There is no combination of finite parameters for which the magnitude of
$S_{\perp}$ diverges, so the modulation of the oscillation frequency does
not cause any resonant behavior.
}
...
\begin{align}
    \rd{S_{\perp}}{t} &\approx i\overline{\Omega}_{\rm e}S_\perp
        - i\cos \bar{\theta}_{\rm e} \sin \p{\Delta I_{\rm eN}} \Omega_{\rm eN}
            \cos \p{N\Omega_{\rm LK} t}.
\end{align}
We can approximate $\cos \p{N\Omega_{\rm LK}t} \approx e^{iN\Omega_{\rm LK} t} /
2$, as the $e^{-iN\Omega_{\rm LK} t}$ component is far from resonance. Then the
equation of motion has the solution
\begin{align}
...
\end{align}
The instantaneous oscillation amplitude $\abs{\Delta \bar{\theta}_{\rm e}}$ can
be bound by
\begin{equation}
    \abs{\Delta \bar{\theta}_{\rm e}} \sim \frac{1}{2}
        \abs{\frac{\sin \p{\Delta I_{\rm eN}} \Omega_{\rm eN}}{
             \overline{\Omega}_{\rm e} - N\Omega_{\rm LK}}}.
        \label{eq:harmonic_dqeff}
\end{equation}
\textcolor{Corr}{
We see that there resonances when $\overline{\Omega}_{\rm e} = N \Omega_{\rm
LK}$. Numerically, we find that $\overline{\Omega}_{\rm e} < \Omega_{\rm LK}$
for the region of parameter space relevant to LK-induced mergers (see
Fig.~\ref{fig:dWs}), so we focus on the effect of the $N = 1$ resonance. If, for
the entire inspiral, $\overline{\Omega}_{\rm e} < \Omega_{\rm LK}$ by a
sufficient margin that Eq.~\eqref{eq:harmonic_dqeff} remains small,} then the
conservation of $\bar{\theta}_{\rm e}$ cannot be significantly affected by this
resonance. \textcolor{Corr}{For the fiducial simulation,
Fig.~\ref{fig:4sim_90_350_supp} shows the ratio $\overline{\Omega}_{\rm e} /
\Omega_{\rm LK}$ in the middle panel (black) and the amplitude of oscillation of
$\bar{\theta}_{\rm e}$ due to the $N = 1$ resonance
[Eq.~\eqref{eq:harmonic_dqeff}] in the bottom panel (blue).} We see that
\textcolor{Corr}{the system is never close to the resonant condition
...
\textcolor{Corr}{It is surprising that the contribution of the resonant terms to
the instantaneous $\Delta \bar{\theta}_{\rm e}$ in the $\mathcal{A} \simeq 1$
regime is dominant over that of the nonadiabatic effect. In the previous
section, we showed that neglecting resonant terms still allows for an accurate
prediction of the final deviation $\abs{\Delta \bar{\theta}_{\rm e}}_{\rm f}$,
implying the nonadiabatic effect must instead be more important in determining
the final deviation. This requires that a $\abs{\Delta \bar{\theta}_{\rm e}}$ of
up to a few degrees due to the $N \geq 1$ Fourier harmonics not affect the final
nonconservation of $\bar{\theta}_{\rm e}$ by more than $\sim 0.01^\circ$. This
differs from the nonadiabatic case, where we find $\abs{\Delta \bar{\theta}_{\rm
e}}_{\rm f} \sim \max \abs{\Delta \bar{\theta}_{\rm e}}$. We do not have an
...
\begin{figure}
    ...
    for the fiducial parameter regime. \textcolor{Corr}{In the fiducial
    parameter regime, where where only systems with $I_{\rm i} \approx 90^\circ$
    will merge within a Hubble time, $I(e_{\min}) \sim 90^\circ$ for most of the
    evolution (see Fig.~\ref{fig:4sim_90_350}) until $e_{\min} \approx 1$ is
    satisfied. This plot then shows that $\overline{\Omega}_{\rm e} \lesssim
    0.5 \Omega_{\rm LK}$ is a general feature of LK-induced mergers, as is the
    case for the fiducial simulation (see
    Fig.~\ref{fig:4sim_90_350_supp}).}}\label{fig:dWs}
\end{figure}
...
the inner binary is sufficiently \textcolor{Corr}{compact} ($\sim
0.1\;\mathrm{AU}$) that it can merge in isolation via GW radiation.
\textcolor{Corr}{We consider a set of parameters that has the same $t_{\rm LK,
i}$ as the system studied in LL17 but has a tertiary SMBH\@: $m_1 = m_2 =
30M_{\odot}$, $a_{\rm i} = 0.1\;\mathrm{AU}$, $e_{\rm i} = 10^{-3}$, $m_3 = 3
\times 10^7 M_{\odot}$, $\tilde{a}_{\rm out} = 300\;\mathrm{AU}$, and $e_{\rm
out} = 0$.}
...
\begin{figure}
    ...
    = 3 \times 10^{7}M_{\odot}$, $a_{\rm i} = 0.1\;\mathrm{AU}$, $e_{\rm i} =
    ...
    $\theta_{\rm sl, f}$ and $\theta_{\rm e, i}$, and we denote this range with
    a broad red shaded interval.}\label{fig:bin_comp}
\end{figure}

First, we illustrate the $\theta_{\rm sl, f}$ distribution obtained via
numerical simulation, shown as the blue dots in Fig.~\ref{fig:bin_comp}.
\textcolor{Corr}{The prediction assuming conservation of $\bar{\theta}_{\rm e}$
is shown in the red solid line. Good agreement is observed both when no
eccentricity excitation occurs ($I_{\rm i} \lesssim 50^\circ$ and $I_{\rm i}
\gtrsim 130^\circ$) and when $\abs{\mathcal{A}} \gg 1$ ($80^\circ \lesssim
I_{\rm i} \lesssim 100^\circ$).} However, we see in Fig.~\ref{fig:bin_comp} that
for intermediate inclinations $I_{\rm i} \in [50, 80]$ and $I_{\rm i} \in [100,
130]$, \textcolor{Corr}{$\theta_{\rm sl, f}$ varies over a large range and does
not agree with the prediction of $\bar{\theta}_{\rm e}$ conservation. This
pattern of deviations is different than that seen in the} fiducial parameter
regime (Fig.~\ref{fig:qslscan}), as this inclination regime corresponds to
neither the fastest nor slowest merging systems.

We attribute the origin of these deviations to resonance interactions.
\textcolor{Corr}{Fig.~\ref{fig:dWs_inner} illustrates that these intermediate
inclinations are closer to the $\overline{\Omega}_{\rm e} = \Omega_{\rm LK}$
resonance than inclinations very close to $90^\circ$. In addition, as the inner
binary coalesces under GW radiation, $e_{\min}$ becomes larger (e.g.\ see
...
\textcolor{Corr}{For initial inclinations closer to $90^\circ$, these systems
start at the edge of the $N = 1$ resonance and quickly evolve away from it (as
$e_{\min}$ increases and causes $\overline{\Omega}_{\rm e} / \Omega_{\rm LK}$
to decrease), so the cumulative effect of the resonance interaction is well
predicted by evaluating Eq.~\eqref{eq:harmonic_dqeff} for $N = 1$ at the initial
conditions. This is denoted by the narrow red shaded region surrounding the red
line near $I_{\rm i} = 90^\circ$ in Fig.~\ref{fig:bin_comp}. We see that the
scatter is well predicted for these weakly-resonant conditions. We empirically
choose the transition between ``weakly'' and ``strongly'' resonant systems when
the $\abs{\Delta \bar{\theta}_{\rm e}}$ predicted by
Eq.~\eqref{eq:harmonic_dqeff} exceeds $7^\circ$.}
...
\begin{figure}
    ...
    regime. \textcolor{Corr}{In the LK-enhanced regime, all $I_{\rm i}$ merge
    within a Hubble time, and it is clear that $\overline{\Omega}_{\rm e}
    \approx \Omega_{\rm LK}$ can be satisfied for a wide range of inclinations.
    However, $e_{\max} \lesssim 0.8$ for these inclinations is moderate compared
    to the LK-induced regime.}}\label{fig:dWs_inner}
\end{figure}

\textcolor{Corr}{To understand the general characteristics of systems that
interact strongly with resonances, we examine the quantities in
Eq.~\eqref{eq:harmonic_dqeff}}:
\begin{itemize}
...
\end{itemize}
However, \textcolor{Corr}{the timescales over which $\mathcal{A}$ increases and
$e_{\min}$ decreases are comparable (see Fig.~\ref{fig:4sim_90_350})}. This
implies that, if $\mathcal{A} \ll 1$ initially, which is the case for LK-induced
mergers, then $e_{\min}$ will be very close to unity when $\mathcal{A}$ grows to
be $\simeq 1$, and the contribution predicted by Eq.~\eqref{eq:harmonic_dqeff}
will remain small throughout the entire evolution. \textcolor{Corr}{On the other
hand, only if $\mathcal{A} \simeq 1$ and $e_{\min} \ll 1$ initially, as is the
...
$\bar{\theta}_{\rm e, f} = \bar{\theta}_{\rm e, i}$,
\begin{equation}
    \theta_{\rm sl, f} = \bar{\theta}_{\rm e, i}.
\end{equation}

First, we consider the case where $\bm{S}_{\rm i} = \uv{\overline{L}}_{\rm i}$.
Then $\bar{\theta}_{\rm e, i} = \abs{I_{\rm tot, i} - \bar{I}_{\rm tot,e, i}}$
(see Fig.~\ref{fig:3vec_eta}), and so
\begin{equation}
    \theta_{\rm sl, f} = \abs{I_{\rm tot, i} - \bar{I}_{\rm e, i}}.
...
of $\Omega_{\rm L}$ changes over a LK cycle when $90^\circ < I_{\rm i} <
I_{\lim}$. We find that
\begin{equation}
    \theta_{\rm sl, f} =
    \begin{cases}
        I_{\rm tot, i} & I_{\rm i} < I_{\rm c},\\
        180^\circ - I_{\rm tot, i} & I_{\rm i} > I_{\rm c}.
    \end{cases}
\end{equation}

More generally, so long as $\mathcal{A} \ll 1$ initially, we can specify the
initial spin orientation by $\theta_{\rm s,tot, i}$, the initial angle
between $\bm{S}$ and $\bm{L}_{\rm tot}$, giving
\begin{equation}
    \theta_{\rm sl, f} =
    \begin{cases}
        \theta_{\rm s,tot, i} & I < I_{\rm c},\\
        180^\circ - \theta_{\rm s,tot, i} & I > I_{\rm c}.
    \end{cases}
\end{equation}
...
accurately predicts $\theta_{\rm sl, f}$ when $\theta_{\rm sl, i} = 0^\circ$ for
...
Section~\ref{ss:effect} that when $\eta = 0$, $\bm{L}$ is fixed when $I_{\rm i}
= 90^\circ$, as $\rdil{\ascnode}{t} = \rdil{I}{t} = 0$. For nonzero $\eta$,
$\rdil{\ascnode}{t}$ and $\rdil{I_{\rm tot}}{t}$ are not zero at $I_{\rm i} =
I_{\rm c}$.
...
Section~\ref{s:lk_enhanced} but use a tertiary companion with $m_3 =
...
companion. We study cases where the tertiary companion is either a supermassive
black hole (SMBH) or a stellar mass black hole (BH). We show that, when the
tertiary orbit is circular and octupole effects are unimportant, the spin
vectors of the inner BHs obey the simple equation of motion
Eq.~\eqref{eq:dsdt_weff}. Analysis of this equation yields the following
conclusions:
\begin{itemize}
    ...
    \item When the resonant condition $\bar{\Omega}_{\rm e} \approx N\Omega_{\rm
        ...
        oscillation amplitude. This estimate both demonstrates that the
        resonance is unimportant for ``LK-induced'' mergers and tentatively
        explains the scatter in $\theta_{\rm sl, f}$ seen by LL17.
\end{itemize}

In the beginning of the paper, we pointed out that LL18 found that the final
spin-orbit misalignment angle $\theta_{\rm sl, f}$ exhibits a $90^\circ$
attractor (Fig.~\ref{fig:qslscan}) when the octupole effect is negligible. Our
theory shows that this attractor is a result of their taking $\theta_{\rm sl, i}
= 0^\circ$, as the initial and final spin orientations are related via
...
LK-induced scenario, a primordial $\theta_{\rm sl, i} \approx 0^\circ$ would be
...
distribution is largely unrelated to the initial $\bar{\theta}_{\rm e, i}$
\citep{bin2}.
...
$\theta_{\rm sl, f}$ and $\bar{\theta}_{\rm e, i}$. However, \citet{bin2} found
...
In this appendix, we provide an alternative approach to analyzing the dynamics
of the system that provides results complementary to those in the main text.
Through a combination of analytical and numerical results, we argue that the
results of the main text provide a complete and description of the system. We
again work in the corotating frame and neglect GW dissipation, so the equation
of motion is given by Eq.~\eqref{eq:dsdt_weff}. If we define the matrix operator
satisfying $\tilde{\bm{A}}\bm{S} = \bm{\Omega}_{\rm e} \times \bm{S}$, then the
...
In passing, we take note of one difference between the resonance models in
Section~\ref{s:harmonic} and Appendix~\ref{app:a2}. If only one harmonic exists,
e.g.\ if $\bm{\Omega}_{\rm eN} = 0$ for $N \geq 2$, then
Eq.~\eqref{eq:harmonic_dqeff} predicts that there should only be one resonance,
at $\overline{\Omega}_{\rm e} = \Omega_{\rm LK}$, while Eq.~\eqref{eq:app_Mgen}
suggests there should still be multiple resonances when $\overline{\Omega}_{\rm
e} = N\Omega_{\rm LK}$. Tentative numerical experiments suggest the latter
result is correct. In the problem studied in this paper, this distinction is
unimportant since $\overline{\Omega}_{\rm e}$ does not exceed a few $\Omega_{\rm
LK}$, and the first few harmonics $\bm{\Omega}_{\rm eN}$ are quite similar (the
characteristic scale in $N$ over which the $\bm{\Omega}_{\rm eN}$ change is $1 /
j(e_{\max}) \gg 1$).
...
\subsection{Quantitative Effect}\label{eq:app_num}
...
regime ($m_1 = m_2 = m_3 = 30M_{\odot}$, $a_{\rm i} = 0.1\;\mathrm{AU}$, $e_{\rm
i} = 10^{-3}$,$\tilde{a}_{\rm out} = 3\;\mathrm{AU}$). We still neglect GW
...
then $\bar{\theta}_{\rm e}$ oscillates with amplitude $2\Delta I_{\rm m}$. Thus,
we can infer $\Delta I_{\rm m}$ in the vicinity of each $\overline{\Omega}_{\rm
e} = N\Omega_{\rm LK}$ resonance from Eq.~\eqref{eq:harmonic_dqeff}. If the
resonances are well spaced, the total $\Delta I_{\rm m}$ is well approximated by
the sum of the contribution from each $N$ harmonic, so
\begin{equation}
    \Delta I_{\rm m} \sim
        \sum\limits_{N = 1}^{\infty} \abs{\frac{\sin \Delta I_{\rm eN}
...
numeric $\Delta I_{\rm m}$ rather well except for the exact shape near $I =
90^\circ$. This supports our assertion that $\bm{R}$ deviates from
$\hat{\overline{\bm{\Omega}}}_{\rm e}$ at resonances $\overline{\Omega}_{\rm e}
\approx N \Omega_{\rm LK}$, and the misalignment is captured by
Eq.~\eqref{eq:harmonic_dqeff}.
...
\begin{figure}
    ...
    parameter regime ($m_1 = m_2 = m_3 = 30M_{\odot}$, $a_{\rm i} =
    0.1\;\mathrm{AU}$, $e_{\rm i} = 10^{-3}$,$\tilde{a}_{\rm out} =
    ...
    scaling of the misalignment angle near resonances is generally well captured
    by our analytic formula, but there are some small discrepancies near
    $90^\circ$, and the numerical misalignment angle crosses $0$ within the $N =
    1$ resonance, which is not predicted by our simple
