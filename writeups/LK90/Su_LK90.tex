...
the $\theta_{\rm sl}^{\rm f} = 90^\circ$ attractor as shown in
Fig.~\ref{fig:qslscan}.
...
\begin{align}
    ...
    \overline{\Omega}_{\rm SL} &\propto \frac{1}{a}.
\end{align}
...
i}$ and $t = t_{\rm f}$, i.e.
\begin{equation}
    \bar{\theta}_{\rm e}^{\rm f} \simeq \bar{\theta}_{\rm e}^{\rm i}.
...
$\bar{\theta}_{\rm e}^{\rm f} = \theta_{\rm sl}^{\rm f}$.
Eq.~\eqref{eq:4_4_qeffcons} then gives
\begin{equation}
    \theta_{\rm sl}^{\rm f} \simeq \bar{\theta}_{\rm e}^{\rm i},
...
$\overline{\bm{\Omega}}_{\rm e}$ by $\bar{\theta}_{\rm e}^{\rm i} = \abs{I_0 -
{\bar{I}_{\rm e}}^{\rm i}}$, where $I_0$ is the initial inclination angle
between $\bm{L}$ and $\bm{L}_{\rm out}$ and ${\bar{I}_{\rm e}}^{\rm i}$ is the
initial value of $\bar{I}_{\rm e}$. Thus, adiabatic invariance implies
\begin{equation}
    \theta_{\rm sl}^{\rm f} \simeq \abs{I_0 - {\bar{I}_{\rm e}}^{\rm i}}.
\end{equation}
...
out}$ (for $I_0 < 90^\circ$) or antiparallel to $\bm{L}_{\rm out}$ (for $I_0 >
90^\circ$). Thus,
\begin{equation}
    \theta_{\rm sl}^{\rm f} =
    \begin{cases}
    ...
    \end{cases}
\end{equation}
Since LK-induced mergers necessarily require $I_0$ close to $90^\circ$, we find
that $\theta_{\rm sl}^{\rm f}$ is ``attracted'' to $90^\circ$.
...
LK-induced mergers), $\overline{\Omega}_{\rm e}^{\rm i}$ is parallel to $\pm
\bm{L}_{\rm out}$. Suppose the initial inclination between $\bm{S}$ and
$\bm{L}_{\rm out}$ is $\theta_{\rm s, out}^{\rm i}$, then $\bar{\theta}_{\rm
e}^{\rm i} = \theta_{\rm s, out}^{\rm i}$ or $180^\circ - \theta_{\rm s,
out}^{\rm i}$ (depending on whether $I_0 < 90^\circ$ or $I_0 > 90^\circ$). Thus,
\begin{equation}
    \theta_{\rm sl}^{\rm f} \simeq
    \begin{cases}
        \theta_{\rm s, out}^{\rm i} & I_0 < 90^\circ,\\
        180^\circ - \theta_{\rm s, out}^{\rm i} & I_0 > 90^\circ,
    \end{cases}
\end{equation}
...
sl}^{\rm f} = 0^\circ$. This can be understood: $I_0 = 90^\circ$ gives
...
$\theta_{\rm sl}^{\rm i} = 0$, so $\theta_{\rm sl}^{\rm f} = 0$.

Finally, Fig.~\ref{fig:qslscan} shows that the actual $\theta_{\rm sl}^{\rm f}$
...
\begin{figure}
    ...
    near $I_0 = 90^\circ$ is good when accounting for the effects of a finite
...
\section{Stellar Mass BH Triples}

In this section, we extend our predictions to systems where all three masses are
comparable and $\eta \equiv L / L_{\rm out}$ is not negligible. We still
consider the simplified limit where $e_{\rm out} \ll 1$ and the octupole-order
effects are negligible. The equations of motion are given in \citet{bin_diego}.

When $\eta \neq 0$, $\bm{L}_{\rm out}$ is no longer fixed, but the total angular
momentum $\bm{L}_{\rm tot} \equiv \bm{L} + \bm{L}_{\rm out}$ is. We choose
the coordinate system with $\uv{z} = \uv{L}_{\rm tot}$ and go to the corotating
frame with $\bm{L}$ about $\bm{L}_{\rm tot}$, in which
\begin{equation}
    \p{\rd{\bm{S}}{t}}_{\rm rot} = \bm{\Omega}_{\rm e} \times \bm{S},
\end{equation}
where
\begin{align}
    \bm{\Omega}_{\rm e} &\equiv \Omega_{\rm SL}\hat{L}
        + \Omega_{\rm L} \frac{\bm{L}_{\rm tot}}{L_{\rm out}},\nonumber\\
        &= \overline{\Omega_{\rm SL} \sin I_{\rm tot}}\uv{x}
            + \p{\overline{\Omega}_{\rm L}\frac{L_{\rm tot}}{L_{\rm out}}
                + \overline{\Omega_{\rm SL}\cos I_{\rm tot}}}\uv{z},
\end{align}
where $-\Omega_{\rm L} \bm{L}_{\rm tot} / L_{\rm out}$ is the rate of precession
of $\bm{L}$ about $\bm{L}_{\rm tot}$. The results of Section~\ref{s:fast_merger}
suggest then that the angle $\bar{\theta}_{\rm e}$ is an adiabatic invariant,
where $\bar{\theta}_{\rm e}$ is given by
\begin{equation}
    \cos \bar{\theta}_{\rm e} \equiv
        \frac{\overline{\bm{\Omega}}_{\rm e}}{\overline{\Omega}_{\rm e}}
            \cdot \overline{\bm{S}},
\end{equation}
where bars denote averaging over a LK cycle.

Similarly to the $\eta \ll 1$ case, $\bar{\theta}_{\rm e}^{\rm f} = \theta_{\rm
sl}^{\rm f}$ once the inner binary is sufficiently compact, as $\mathcal{A} \gg
1$ for sufficiently small $a$. This implies that
\begin{equation}
    \theta_{\rm sl}^{\rm f} = \bar{\theta}_{\rm e}^{\rm i}.
\end{equation}

First, we consider the case where $\bm{S}^{\rm i} \parallel \bm{L}^{\rm i}$.
This implies $\bar{\theta}_{\rm e}^{\rm i} = \abs{I_{\rm tot}^{\rm i} -
\bar{I}_{\rm e}^{\rm i}}$ (see Fig.~TODO), and
\begin{equation}
    \theta_{\rm sl}^{\rm f} = \abs{
        I_{\rm tot}^{\rm i} - \bar{I}_{\rm e}^{\rm i}}.
\end{equation}
Suppose additionally that initially $\abs{\overline{\Omega}_{\rm L}} \gg
\abs{\overline{\Omega}_{\rm SL}}$, then $\overline{\Omega}_{\rm e}^{\rm i}$ is
either parallel or anti-parallel to $\bm{L}_{\rm tot}$ depending on whether
$\overline{\Omega}_{\rm L}$ is positive or negative. The sign of
$\overline{\Omega}_{\rm L}$ is positive if $I_0 < I_{\rm 0, \lim}$ and negative
if $I_0 > I_{\rm 0, \lim}$, where \citep{bin2}
\begin{equation}
    \cos I_{\rm 0, \lim} = \frac{\eta}{2}\p{\frac{4}{5}j_{\lim}^2 - 1},
\end{equation}
...
\begin{equation}
    \frac{3}{8}\p{j_{\lim}^2 - 1}\s{-3 + \frac{\eta^2}{4}
        ...
        = 0.
\end{equation}
This allows us to make prediction
\begin{equation}
    \theta_{\rm sl}^{\rm f} =
    \begin{cases}
        I_{\rm tot}^{\rm i} & I_0 < I_{\rm 0, \lim},\\
        180^\circ - I_{\rm tot}^{\rm i} & I_0 > I_{\rm 0, \lim}.
    \end{cases}
\end{equation}
Again, we can compare this to the results of numerical simulations in
Fig.~\ref{fig:finite_qslfscan} in the LK-induced regime and in
...
\begin{figure}
    ...
    $\tilde{a}_3 = 4500\;\mathrm{AU}$.}\label{fig:finite_qslfscan}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{LK90_plots/8bin_comp.png}
    \caption{Same as Fig.~\ref{fig:bin_comp} but for $m_3 = 30M_{\odot}$ and
    $\tilde{a}_3 = 3\;\mathrm{AU}$.}\label{fig:finite_bin_comp}
\end{figure}

More generally, so long as $\mathcal{A} \ll 1$ initially, we can specify the
initial spin orientation by $\theta_{\rm s, tot}^{\rm i}$, the initial angle
between $\bm{S}$ and $\bm{L}_{\rm tot}$, giving
\begin{equation}
    \theta_{\rm sl}^{\rm f} =
    \begin{cases}
        \theta_{\rm s, tot}^{\rm i} & I < I_{\rm 0, \lim},\\
        180^\circ - \theta_{\rm s, tot}^{\rm i} & I > I_{\rm 0, \lim}.
    \end{cases}
\end{equation}
...
\section{Justification of Using $\overline{\bm{\Omega}}_{\rm e}$}

We seek to compute the monodromy matrix $\tilde{\bm{M}}$ in closed form.
Consider the EOM
\begin{equation}
...
rotation about $\uv{\overline{\bm{\Omega}}}_{\rm e}$ by angle $2\pi N$).

This analysis shows a few points that complement the results in the main text:
\begin{itemize}
    \item It gives a different interpretation to the resonant condition
        $\overline{\Omega}_{\rm e} \approx N\Omega_{\rm LK}$.

    \item The commutator vanishes for the same conditions as are required in the
        main text: the Fourier components must be modestly misaligned from the
        averaged component and have substantial magnitudes.

    \item It gives an explicit, though approximate, solution for the monodromy
        matrix.

    \item It proves that the averaged component of $\bm{\Omega}_{\rm e}$ can
        indeed be analyzed separately from the higher harmonics, as otherwise
        it's conceivable that higher harmonics can generate large
        $\bar{\theta}_{\rm e}$ variations on much longer timescales than $T_{\rm
        LK}$.
\end{itemize}
