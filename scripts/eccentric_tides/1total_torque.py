...
def plot_ecc(w_s=0, num_pts=2000, emax=0.99):
    ...
    plt.semilogy(e_vals, sign * totals, ls='', marker='o', mfc='none', mec='k',
                 ms=ms, label='Sum')
    plt.semilogy(e_vals, sign * totals_integ, 'gx', ms=ms, label='Integral')
    plt.semilogy(e_vals, sign * torque, 'b', lw=lw, alpha=af,
                 label='Analytic')
    plt.xlabel(r'$e$')
...
def plot_spin(e=0.9, num_pts=1000):
    ...
    plt.plot(w_vals / w_p, totals_integ, 'gx',
    ...
    # use higher resolution for analytic curve
    w_vals = np.linspace(-120, 200, 1000)
...
def plot_energy(w_s=0, num_pts=2000, emax=0.99):
    ...
    e_vals = np.linspace(1e-3, emax**2, 99)**(1/2)
    totals = []
    totals_integ = []
    ...
    plt.semilogy(e_vals, sign * totals, ls='', marker='o', mfc='none', mec='k',
                 ms=ms, label='Sum')
    plt.semilogy(e_vals, sign * totals_integ, 'gx', ms=ms, label='Integral')
    plt.semilogy(e_vals, sign * (term0 + term2) / 2, 'b', lw=lw, alpha=af,
                 label='Analytic')
...
def plot_spin_energy(e=0.9, num_pts=300):
    ...
    plt.plot(w_vals / w_p, totals_integ,
                 'gx', ms=ms, alpha=af, label='Integral')
    ylims = plt.ylim()
    ...
    plt.plot(w_vals / w_p, edotin, 'b', ms=ms, alpha=af, label='Analytic')
    plt.yscale('symlog', linthresh=1.2e9)
    plt.ylim(ylims)
