...
def run_one(I_deg=90.355, e=0.001, fn_template='1sim_%s',
    ...
    if plot:
        plot_func(ret, fn,
                  m1, m2, m3, a0, a2, e2, I,
                  getter_kwargs=getter_kwargs,
                  plot_slice=np.where(ret.t > ret.t[-1] - 2)[0],
                  )
    L, _, s = to_vars(ret.y)
...
def plot_many(ens_fn='1ensemble', plot=True, **kwargs):
    ''' run all the way to merger, measure q_sl at the end and plot '''
    q_slfs = []
    tfs = []
    I_degs = np.arange(90.05, 90.3, 0.002)
    for I_deg in I_degs:
        q_slf, ret = run_one(I_deg=I_deg, plot=False, **kwargs)
        q_slfs.append(q_slf)
...
if __name__ == '__main__':
    ...
    for idx, wsl_mult in enumerate([0.1, 1/3, 1, 3, 10]):
        mkdirp('wsl_%d' % idx)
        q_slfs = plot_many(a_f=0.3, fn_template='wsl_' + str(idx) + '/1sim_%s',
                           wsl_mult=wsl_mult, plot=False)
        plt.plot(q_slfs, 'r', label=r'$%.1f \times \Omega_{rm SL}$' % wsl_mult)
    plt.xlabel(r'$I$ (Deg)')
