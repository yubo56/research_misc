...
\end{align}
Thus, we should expect the dominant term to come at $N \sim \Omega_p$. This
indeed very nearly maximizes $F_{N2}$ but probably won't maximize $\tau_N$.
...
\subsection{Hansen Coefficient Fit}
...
p / N_{\max} \simeq \frac{p\Omega_0}{\Omega_p}$.
...
obvious scaling for $a$ as above. The final difficulty is understanding how $C$
scales.
\begin{figure}[t]
    \centering
...
\begin{align}
    \tau &= T_0 C(r_c) \sum\limits_{N = -\infty}^\infty
            F_{N2}^2 \mathrm{sgn}\p*{N\Omega - 2\Omega_s}
                \sigma_c^{8/3},\\
        &= T_0 C(r_c) \sum\limits_{N = 1}^\infty
            \s*{F_{N2}^2 \mathrm{sgn}\p*{N\Omega - 2\Omega_s}
                \sigma_c^{8/3} +
                F_{-N2}^2 \mathrm{sgn}\p*{-N\Omega - 2\Omega_s}
                    \sigma_c^{8/3}\p*{N \to -N}},\\
        &\approx T_0 C(r_c)\s*{
            \int\limits_0^\infty C_+^2N^{2p_{+}} e^{-2N / a_{+}}
                \mathrm{sgn}\p*{N\Omega - 2\Omega_s}
                \sigma_c^{8/3}\;\mathrm{d}N +
             \int\limits_0^\infty C_-^2N^{2p_{-}} e^{-2N / a_{-}}
                \mathrm{sgn}\p*{-N\Omega - 2\Omega_s}
                \sigma_c^{8/3}\p*{N \to -N}\;\mathrm{d}N}.
\end{align}
At this point, let's specialize to two regimes:
\begin{itemize}
    \item Let $2\Omega_s \ll N_{\max} \Omega$, then the sign term is just the
        sign of $N$, and $\sigma_c = \p*{N\Omega}^{8/3}$. This gives
        \begin{align}
            \tau &\approx T_0 C(r_c) \s*{
    ...
    \item Alternatively, let $2\Omega_s \gg N_{\max}\Omega$, then the sign is
        just always negative and $\sigma_c \approx 2\Omega_s$, and we find
        \begin{align}
            \tau &\approx T_0 C(r_c) \p*{2\Omega_s}^{8/3}\s*{
                -C_+^2\int\limits_0^{\infty}
                    N^{2p_+}e^{-2N/a_+}\;\mathrm{d}N
                - C_-^2 \int\limits_0^{\infty}
                    N^{2p_-}e^{-2N/a_-}\;\mathrm{d}N},\\
                &\approx T_0C(r_c)\p*{2\Omega_s}^{8/3}\s*{
