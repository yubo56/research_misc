...
Key results are Eq.~\eqref{eq:traveling_torque}. Note that the technique used to
constrain the prefactor on the $\Omega_s \ll N_{\max}\Omega$ is general and
necessary for any of these results where we need to expand $\abs*{N\Omega -
m\Omega_s}^p$ in two regimes.
...
This lets us update our torque formula. Recall we want to sum
\begin{equation}
    \tau = T_0 \hat{C}(r_c)
            \sum\limits_{N = -\infty}^\infty
                F_{N2}^2 \sgn\p*{N - 2\frac{\Omega_s}{\Omega}}
...
\begin{itemize}
    ...
    \item Consider $2\Omega_s \ll N_{\max} \Omega$, then the sign is the
        ...
        This can be solved to find that $\beta =
        \p*{\frac{\Gamma\p*{23/3}}{4!}}^{3/8}/4 \approx 1.38$. Thus,
        \begin{align}
            \hat{\tau}
...
\begin{itemize}
    ...
    \item Again, in the other limit where $2\Omega_s \ll N_{\max}\Omega$, we
        ...
        \begin{equation}
            \abs*{2\Omega_s}^{8/3} \sum\limits_N F_{N2}^2 N
                = \abs*{\frac{2\Omega_s}{\beta N_{\max}\Omega}}^{8/3}
                    \sum\limits_N F_{N2}N^{11/3}.
        \end{equation}
        This requires a bit more work but should be finishable TODO
\end{itemize}
