...
% \textbf{Note: the below uses the wrong form for $N_{peri}$, we should fix it
% eventually.} Now, what about when $\delta e \equiv 1 - e \ll 1$? Let's expand
% \begin{align}
%     2 \p{1
...
host star. If we assume nearly maximal rotation, take characteristic $\rho_c
/ \bar{\rho}_c \sim 3/4$, and use mass ratio $q = 6.3$, then let's solve
\begin{align}
    \dot{E}_{\rm obs} &= \scinot{1.173}{27}\;\mathrm{W},\\
        &= \beta_2 \frac{G\p{M / q}^2r_c^5}{a^6}
            \sigma_c^{8/3}f(0.0469)\Omega_o
                \p{\frac{\dot{E}_{\rm in}}{\hat{T}\Omega}}_{e = 0.808},\\
    \abs{\dot{E}_{\rm obs}} &\leq \beta_2 \frac{G\p{M / q}^2r_c^5}{a^6}
            \sigma_c^{8/3}f(0.0469)\Omega_o
                \p{\scinot{3}{11}},\\
        &\leq \scinot{2.442}{27}\;\mathrm{W}
            \p{\frac{M}{8.8M_{\odot}}}^2\p{\frac{r_c}{1.472 R_{\odot}}}^5.
\end{align}

The procedure is now as follows: we seek MESA models for which the $L$ and
$T_{\rm eff}$ agree with the observations. This should pick out a particular $M$
for each $\alpha_{\rm ov}$ (possibly the same $M$). We then seek to find the
minimum $\alpha_{\rm ov}$ such that $\dot{E}_{\rm obs}$ is feasible.

To write this differently, consider $M_{\rm obs}\p{\alpha_{\rm ov}}$,
constrained by the observed $L$ and $T_{\rm eff}$. We also know though
$r_c\p{\alpha_{\rm ov}, M}$, constrained via the same simulations. So if we plot
simulations for different $\alpha_{\rm ov}$ in $(M, r_c)$ space, and overplot
the constraint that it must be able to explain the observed orbital decay, we
