...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{../scripts/eccentric_tides/1totals_s_0_9.png}
...
\url{https://iopscience.iop.org/article/10.1088/0004-637X/741/1/12/meta}}.% chktex 8
...
values above. See plots for comparison of MESA data. % TODO
...
\begin{align}
    ...
    \frac{\rho_c}{\bar{\rho_c}} \p{1 - \frac{\rho_c}{\bar{\rho}_c}}^2
            &\approx 0.0438,\\
    a &\approx 126R_{\odot},\\
% 8 * (1 + 15 * e**2/2 + 45 * e**4/8 + 5 * e**6/16) / (5 * (1 + 3 * e**2 + 3 * e**4/8) * (1 - e**2)**(3/2))
    N_{\max} &= \frac{8f_2}{5f_5\p{1 - e^2}^{3/2}}
        \approx 21.016,\\
% braces \approx 19550.5
% (1 + 3 * e**2 + 3 * e**4/8) * 21 * 5 * gamma(14/3) / (2**(1/3) * (1 - e**2)**(9/2)) * 0.0438
    \frac{r_c}{a} &\geq
            \s{-\frac{(1 + q)^2}{6\pi}
...
https://www.mdpi.com/2218-2004/7/2/41} seem to suggest higher solar % chktex 8
...
% >>> G * (1.4 Msun)^2 * (1.38 Rsun)^5 / (126 Rsun)^6
% 9.306510397682394e+29 kg.m^2.000\s^2.000
% >>> (2 * pi / (3.5 day)) / (G * (3 Msun) / (1.38 Rsun)^3)^(1/2)
% 0.030959311403070736
...
\subsection{Plotting J0045-7319} % chktex 8
...
% 6 * pi / 8.8 * (1.38 / 126)^5 * 0.76 * (1 - 0.76)^2 * (2 * pi / (51 day) / (G * (3 Msun) / (1.38 Rsun)^3)^(1/2))^(8/3)
\begin{equation}
